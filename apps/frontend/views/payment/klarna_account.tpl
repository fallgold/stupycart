    ...
    <table class="radio">
      ...
      <tr class="highlight">
        ...
        <td style="width: 1%;"><?php if ($iso_code_3 == 'NLD') { ?>
          <img src="{$this->url->getBaseUrl()}theme/default/image/klarna_nld_banner.png" />
          <?php } ?></td>
      </tr>
      <?php } ?>
    </table>
...
$('#button-confirm').on('click', function() {
	$.ajax({
		url: '{$this->url->getBaseUrl()}payment/klarna_account/send',
		type: 'post',
		...
		beforeSend: function() {
			...
			$('#payment').before('<div class="attention"><img src="{$this->url->getBaseUrl()}theme/default/image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
		complete: function() {
