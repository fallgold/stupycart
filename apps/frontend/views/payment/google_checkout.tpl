...
$('#button-paypal').bind('click', function() {
	$.ajax({
		url: '{$this->url->getBaseUrl()}payment/google_checkout/send',
		type: 'post',
		data: $('#payment :input'),
		dataType: 'json',		
		beforeSend: function() {
			$('#button-paypal').attr('disabled', true);
			$('#button-paypal').after('<span class="wait">&nbsp;<img src="{$this->url->getBaseUrl()}theme/default/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
