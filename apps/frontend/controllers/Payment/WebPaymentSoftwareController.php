...
class WebPaymentSoftwareController extends \Stupycart\Frontend\Controllers\ControllerBase {
	...
	public function sendAction() {
		...
		$request .= '&CC_NUMBER=' . urlencode(str_replace(' ', '', $this->request->getPostE('cc_number')));
		$request .= '&CC_EXP=' . urlencode($this->request->getPostE('cc_expire_date_month') . substr($this->request->getPostE('cc_expire_date_year'), 2));
		$request .= '&CC_CVV=' . urlencode($this->request->getPostE('cc_cvv2'));
		...
		$request .= '&INVOICE_NUM=' . urlencode($this->session->get('order_id'));
		...
		if ('00' === (string)$xml->response_code) {
			...
			$this->model_checkout_order->updateOrder($this->session->get('order_id'), $this->config->get('web_payment_software_order_status_id'), $message, false);				
