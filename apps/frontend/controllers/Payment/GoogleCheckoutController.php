...
class GoogleCheckoutController extends \Stupycart\Frontend\Controllers\ControllerBase {
	public function indexAction() {
		...
		$this->view->pick('payment/google_checkout');
	...
	public function sendAction() {
		...
		//	$xml .= '   	</merchant-private-data>'; 
			...
			if ($this->cart->hasShipping()) {
				...
				$xml .= '				<flat-rate-shipping name="' . (($_tmp = $this->session->get('shipping_method')) ? $_tmp['title'] : null) . '">';
				$xml .= '					<price currency="' . $this->currency->getCode() . '">' . $this->currency->format((($_tmp = $this->session->get('shipping_method')) ? $_tmp['cost'] : null), $this->currency->getCode(), false, false) . '</price>';
	...
	public function callbackAction() {
		$this->log->write(http_build_query($this->request->getQueryE()));
		$this->log->write(http_build_query($this->request->getPostE()));
		/*	
