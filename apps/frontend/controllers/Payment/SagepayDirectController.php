...
class SagepayDirectController extends \Stupycart\Frontend\Controllers\ControllerBase {
	...
	public function sendAction() {
		...
		$data['VendorTxCode'] = $this->session->get('order_id');
		...
		$data['CardHolder'] = $this->request->getPostE('cc_owner');
		$data['CardNumber'] = $this->request->getPostE('cc_number');
		$data['ExpiryDate'] = $this->request->getPostE('cc_expire_date_month') . substr($this->request->getPostE('cc_expire_date_year'), 2);
		$data['CardType'] = $this->request->getPostE('cc_type');
		$data['TxType'] = $this->config->get('sagepay_direct_transaction');
		$data['StartDate'] = $this->request->getPostE('cc_start_date_month') . substr($this->request->getPostE('cc_start_date_year'), 2);
		$data['IssueNumber'] = $this->request->getPostE('cc_issue');
		$data['CV2'] = $this->request->getPostE('cc_cvv2');
		...
		$data['ClientIPAddress'] = $this->request->getServer('REMOTE_ADDR');
		...
		if ($data['Status'] == '3DAUTH') {
			...
			$this->model_checkout_order->confirm($this->session->get('order_id'), $this->config->get('config_order_status_id'));
			...
			$this->model_checkout_order->updateOrder($this->session->get('order_id'), $this->config->get('sagepay_direct_order_status_id'), $message, false);
	...
	public function callbackAction() {
		if ($this->session->has('order_id')) {
			...
			curl_setopt($curl, CURLOPT_FRESH_CONNECT, 1);
			curl_setopt($curl, CURLOPT_POST, 1);
			curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($this->request->getPostE()));
		...
		return;
				...
				$this->response->redirect($this->url->link('checkout/checkout', '', 'SSL'), true);
		return;
			}
		} else {
