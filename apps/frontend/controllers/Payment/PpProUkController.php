...
class PPProUKController extends \Stupycart\Frontend\Controllers\ControllerBase {
	...
	public function sendAction() {
		...
		$request .= '&NAME=' . urlencode($this->request->getPostE('cc_owner'));
		...
		$request .= '&CLIENTIP=' . urlencode($this->request->getServer('REMOTE_ADDR'));
		$request .= '&EMAIL=' . urlencode($order_info['email']);
		$request .= '&ACCT=' . urlencode(str_replace(' ', '', $this->request->getPostE('cc_number')));
		$request .= '&ACCTTYPE=' . urlencode($this->request->getPostE('cc_type'));
		$request .= '&CARDSTART=' . urlencode($this->request->getPostE('cc_start_date_month') . substr($this->request->getPostE('cc_start_date_year'), - 2, 2));
		$request .= '&EXPDATE=' . urlencode($this->request->getPostE('cc_expire_date_month') . substr($this->request->getPostE('cc_expire_date_year'), - 2, 2));
		$request .= '&CVV2=' . urlencode($this->request->getPostE('cc_cvv2'));
		$request .= '&CARDISSUE=' . urlencode($this->request->getPostE('cc_issue'));
		...
		curl_setopt($curl, CURLOPT_HTTPHEADER, array('X-VPS-REQUEST-ID: ' . md5($this->session->get('order_id') . mt_rand())));
		...
		if ($response_info['RESULT'] == '0') {
			...
			$this->model_checkout_order->updateOrder($this->session->get('order_id'), $this->config->get('pp_pro_uk_order_status_id'), $message, false);
