...
class MoneybookersController extends \Stupycart\Frontend\Controllers\ControllerBase {
	public function indexAction() {
		...
		$this->data['transaction_id'] = $this->session->get('order_id');
		...
		$this->data['language'] = $this->session->get('language');		
	...
	public function callbackAction() {
		...
		if ($order_info) {
			...
			if ($this->config->get('moneybookers_secret')) {
				$hash  = $this->request->getPostE('merchant_id');
				$hash .= $this->request->getPostE('transaction_id');
				$hash .= strtoupper(md5($this->config->get('moneybookers_secret')));
				$hash .= $this->request->getPostE('mb_amount');
				$hash .= $this->request->getPostE('mb_currency');
				$hash .= $this->request->getPostE('status');
				
				$md5hash = strtoupper(md5($hash));
				$md5sig = $this->request->getPostE('md5sig');
			...
			if ($verified) {
				switch($this->request->getPostE('status')) {
					case '2':
						$this->model_checkout_order->updateOrder($order_id, $this->config->get('moneybookers_order_status_id'), '', true);
						break;
					case '0':
						$this->model_checkout_order->updateOrder($order_id, $this->config->get('moneybookers_pending_status_id'), '', true);
						break;
					case '-1':
						$this->model_checkout_order->updateOrder($order_id, $this->config->get('moneybookers_canceled_status_id'), '', true);
						break;
					case '-2':
						$this->model_checkout_order->updateOrder($order_id, $this->config->get('moneybookers_failed_status_id'), '', true);
						break;
					case '-3':
						$this->model_checkout_order->updateOrder($order_id, $this->config->get('moneybookers_chargeback_status_id'), '', true);
						break;
				}
			} else {
