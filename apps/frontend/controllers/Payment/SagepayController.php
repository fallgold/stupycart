...
class SagepayController extends \Stupycart\Frontend\Controllers\ControllerBase {
	public function indexAction() {
		...
		$data['Description'] = sprintf($this->language->get('text_description'), date($this->language->get('date_format_short')), (($_tmp = $this->session->get('order_id')) ? $_tmp : null));
		$data['SuccessURL'] = str_replace('&amp;', '&', $this->url->link('payment/sagepay/success', 'order_id=' . $this->session->get('order_id')));
	...
	public function successAction() {
		if ($this->request->hasQuery('crypt')) {
			$string = base64_decode(str_replace(' ', '+', $this->request->getQueryE('crypt')));
			$password = $this->config->get('sagepay_password');	
			...
			if ($data && is_array($data)) {
				...
				$this->model_checkout_order->confirm($this->request->getQueryE('order_id'), $this->config->get('config_order_status_id'));
				...
				if ($data['Status'] == 'OK') {
					$this->model_checkout_order->updateOrder($this->request->getQueryE('order_id'), $this->config->get('sagepay_order_status_id'), $message, false);
				} else {
					$this->model_checkout_order->updateOrder($this->request->getQueryE('order_id'), $this->config->get('config_order_status_id'), $message, false);
				}
