...
class AuthorizeNetAimController extends \Stupycart\Frontend\Controllers\ControllerBase {
	...
	public function sendAction() {
		...
		$data['x_customer_ip'] = $this->request->getServer('REMOTE_ADDR');
		...
		$data['x_card_num'] = str_replace(' ', '', $this->request->getPostE('cc_number'));
		$data['x_exp_date'] = $this->request->getPostE('cc_expire_date_month') . $this->request->getPostE('cc_expire_date_year');
		$data['x_card_code'] = $this->request->getPostE('cc_cvv2');
		$data['x_invoice_num'] = $this->session->get('order_id');
		...
		if (curl_error($curl)) {
			...
			if ($response_info[1] == '1') {
				if (strtoupper($response_info[38]) == strtoupper(md5($this->config->get('authorizenet_aim_hash') . $this->config->get('authorizenet_aim_login') . $response_info[7] . $this->currency->format($order_info['total'], $order_info['currency_code'], 1.00000, false)))) {
					...
					$this->model_checkout_order->updateOrder($this->session->get('order_id'), $this->config->get('authorizenet_aim_order_status_id'), $message, false);				
				}
