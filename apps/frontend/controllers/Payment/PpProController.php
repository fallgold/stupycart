...
class PPProController extends \Stupycart\Frontend\Controllers\ControllerBase {
	...
	public function sendAction() {
		...
		$request .= '&CREDITCARDTYPE=' . $this->request->getPostE('cc_type');
		$request .= '&ACCT=' . urlencode(str_replace(' ', '', $this->request->getPostE('cc_number')));
		$request .= '&CARDSTART=' . urlencode($this->request->getPostE('cc_start_date_month') . $this->request->getPostE('cc_start_date_year'));
		$request .= '&EXPDATE=' . urlencode($this->request->getPostE('cc_expire_date_month') . $this->request->getPostE('cc_expire_date_year'));
		$request .= '&CVV2=' . urlencode($this->request->getPostE('cc_cvv2'));
		
		if ($this->request->getPostE('cc_type') == 'SWITCH' || $this->request->getPostE('cc_type') == 'SOLO') { 
		...
		$request .= '&IPADDRESS=' . urlencode($this->request->getServer('REMOTE_ADDR'));
		...
		if (($response_info['ACK'] == 'Success') || ($response_info['ACK'] == 'SuccessWithWarning')) {
			...
			$this->model_checkout_order->updateOrder($this->session->get('order_id'), $this->config->get('pp_pro_order_status_id'), $message, false);
