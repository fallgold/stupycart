...
class PerpetualPaymentsController extends \Stupycart\Frontend\Controllers\ControllerBase {
	...
	public function sendAction() {
		...
		$payment_data = array(
			...
			'card_num'      => str_replace(' ', '', $this->request->getPostE('cc_number')),
			'card_cvv'      => $this->request->getPostE('cc_cvv2'),
			'card_start'    => $this->request->getPostE('cc_start_date_month') . substr($this->request->getPostE('cc_start_date_year'), 2),
			'card_expiry'   => $this->request->getPostE('cc_expire_date_month') . substr($this->request->getPostE('cc_expire_date_year'), 2),
			...
			'cust_ip'       => $this->request->getServer('REMOTE_ADDR'),
		...
		if ($response) {
			...
			if (isset($data[0]) && $data[0] == 'A') {
				...
				$this->model_checkout_order->updateOrder($this->session->get('order_id'), $this->config->get('perpetual_payments_order_status_id'), $message, false);
