...
class DownloadController extends \Stupycart\Frontend\Controllers\ControllerBase {
	public function indexAction() {
		if (!$this->customer->isLogged()) {
			$this->session->set('redirect', $this->url->link('account/download', '', 'SSL'));
		...
		$this->model_account_download = new \Stupycart\Common\Models\Account\Download();
		...
		if ($download_total) {
			...
			$this->view->pick('account/download');
	...
	public function downloadAction() {
		...
		if ($this->request->hasQuery('order_download_id')) {
			$order_download_id = $this->request->getQueryE('order_download_id');
		} else {
			$order_download_id = 0;
		}
		...
		if ($download_info) {
			...
			if (!headers_sent()) {
				if (file_exists($file)) {
					...
					$this->model_account_download->updateRemaining($this->request->getQueryE('order_download_id'));
			...
			$this->response->redirect($this->url->link('account/download', '', 'SSL'), true);
		return;
		}
	}
}
