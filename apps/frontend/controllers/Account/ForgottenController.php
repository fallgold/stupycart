...
class ForgottenController extends \Stupycart\Frontend\Controllers\ControllerBase {
	...
	public function indexAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			...
			$this->model_account_customer->editPassword($this->request->getPostE('email'), $password);
		...
		$this->view->pick('account/forgotten');
	...
	protected function validate() {
		...
		} elseif (!$this->model_account_customer->getTotalCustomersByEmail($this->request->getPostE('email'))) {
			$this->error['warning'] = $this->language->get('error_email');
		}
