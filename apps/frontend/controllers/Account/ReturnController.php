...
class ReturnController extends \Stupycart\Frontend\Controllers\ControllerBase { 
	private $error = array();
	
	public function indexAction() {
    	if (!$this->customer->isLogged()) {
      		$this->session->set('redirect', $this->url->link('account/return', '', 'SSL'));
		...
		$this->model_account_return = new \Stupycart\Common\Models\Account\Return();
		...
		$this->view->pick('account/return_list');
	...
	public function infoAction() {
		...
		if (!$this->customer->isLogged()) {
			$this->session->set('redirect', $this->url->link('account/return/info', 'return_id=' . $return_id, 'SSL'));
		...
		if ($return_info) {
			...
			$this->data['breadcrumbs'][] = array(
				...
				'href'      => $this->url->link('account/return/info', 'return_id=' . $this->request->getQueryE('return_id') . $url, 'SSL'),
				'separator' => $this->language->get('text_separator')
			);			
			...
			$results = $this->model_account_return->getReturnHistories($this->request->getQueryE('return_id'));
			...
			$this->view->pick('account/return_info');
	...
	public function insertAction() {
    	...
    	if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_account_return->addReturn($this->request->getPostE());
	  		
			$this->response->redirect($this->url->link('account/return/success', '', 'SSL'), true);
		return;
    	} 
		...
		$this->model_account_order = new \Stupycart\Common\Models\Account\Order();
		
		if ($this->request->hasQuery('order_id')) {
			$order_info = $this->model_account_order->getOrder($this->request->getQueryE('order_id'));
		}
		...
		$this->model_localisation_return_reason = new \Stupycart\Common\Models\Localisation\ReturnReason();
		...
		$this->view->pick('account/return_form');
  	...
  	protected function validate() {
    	if (!$this->request->getPostE('order_id')) {
