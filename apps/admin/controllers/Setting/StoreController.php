...
class StoreController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {
  	...
  	public function insertAction() {
    	...
    	if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
			$store_id = $this->model_setting_store->addStore($this->request->getPostE());
  	...
  	public function updateAction() {
    	...
    	if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
			$this->model_setting_store->editStore($this->request->getQueryE('store_id'), $this->request->getPostE());
			
			$this->model_setting_setting = new \Stupycart\Common\Models\Admin\Setting\Setting();
			
			$this->model_setting_setting->editSetting('config', $this->request->post, $this->request->getQueryE('store_id'));
			
			$this->session->set('success', $this->language->get('text_success'));
			
			$this->response->redirect($this->url->link('setting/store', 'token=' . $this->session->get('token') . '&store_id=' . $this->request->getQueryE('store_id'), 'SSL'), true);
		return;
		}
  	...
  	public function deleteAction() {
		...
		if ($this->request->hasPost('selected') && $this->validateDelete()) {
			foreach ($this->request->getPostE('selected') as $store_id) {
	...
	protected function getList() {
		...
		if ($this->request->hasQuery('page')) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('setting/store', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
							
		$this->data['insert'] = $this->url->link('setting/store/insert', 'token=' . $this->session->get('token'), 'SSL');
		$this->data['delete'] = $this->url->link('setting/store/delete', 'token=' . $this->session->get('token'), 'SSL');	
		...
		$this->data['stores'][] = array(
			...
			'url'      => HTTP_CATALOG,
			'selected' => $this->request->hasPost('selected') && in_array(0, $this->request->getPostE('selected')),
			'action'   => $action
		);
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				'text' => $this->language->get('text_edit'),
				'href' => $this->url->link('setting/store/update', 'token=' . $this->session->get('token') . '&store_id=' . $result['store_id'], 'SSL')
			);
			...
			$this->data['stores'][] = array(
				...
				'name'     => $result['name'],
				'url'      => $result['url'],
				'selected' => $this->request->hasPost('selected') && in_array($result['store_id'], $this->request->getPostE('selected')),
				'action'   => $action
			);
		}	
		...
		$this->view->pick('setting/store_list');
		$this->_commonAction();
	...
	public function getForm() { 
		...
		if (!$this->request->hasQuery('store_id')) {
			$this->data['action'] = $this->url->link('setting/store/insert', 'token=' . $this->session->get('token'), 'SSL');
		} else {
			$this->data['action'] = $this->url->link('setting/store/update', 'token=' . $this->session->get('token') . '&store_id=' . $this->request->getQueryE('store_id'), 'SSL');
		}
				
		$this->data['cancel'] = $this->url->link('setting/store', 'token=' . $this->session->get('token'), 'SSL');
	
		if ($this->request->hasQuery('store_id') && ($this->request->getServer('REQUEST_METHOD') != 'POST')) {
			$this->model_setting_setting = new \Stupycart\Common\Models\Admin\Setting\Setting();
			
      		$store_info = $this->model_setting_setting->getSetting('config', $this->request->getQueryE('store_id'));
    	}
		
		$this->data['token'] = $this->session->get('token');
		
		if ($this->request->hasPost('config_url')) {
			$this->data['config_url'] = $this->request->getPostE('config_url');
		...
		if ($this->request->hasPost('config_ssl')) {
			$this->data['config_ssl'] = $this->request->getPostE('config_ssl');
		...
		$this->view->pick('setting/store_form');
		$this->_commonAction();
	...
	protected function validateForm() {
		...
		if (!$this->request->getPostE('config_url')) {
