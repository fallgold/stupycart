...
class VoucherController extends \Stupycart\Admin\Controllers\ControllerBase {
	...
	public function indexAction() { 
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('voucher', $this->request->getPostE());
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('total/voucher', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('total/voucher', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/total', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('voucher_status')) {
			$this->data['voucher_status'] = $this->request->getPostE('voucher_status');
		} else {
			$this->data['voucher_status'] = $this->config->get('voucher_status');
		}

		if ($this->request->hasPost('voucher_sort_order')) {
			$this->data['voucher_sort_order'] = $this->request->getPostE('voucher_sort_order');
		} else {
			$this->data['voucher_sort_order'] = $this->config->get('voucher_sort_order');
		}

		$this->view->pick('total/voucher');
		$this->_commonAction();
