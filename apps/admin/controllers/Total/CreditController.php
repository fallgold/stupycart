...
class CreditController extends \Stupycart\Admin\Controllers\ControllerBase { 
	private $error = array(); 
	...
	public function indexAction() { 
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('credit', $this->request->getPostE());
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('total/credit', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('total/credit', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/total', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('credit_status')) {
			$this->data['credit_status'] = $this->request->getPostE('credit_status');
		} else {
			$this->data['credit_status'] = $this->config->get('credit_status');
		}

		if ($this->request->hasPost('credit_sort_order')) {
			$this->data['credit_sort_order'] = $this->request->getPostE('credit_sort_order');
		} else {
			$this->data['credit_sort_order'] = $this->config->get('credit_sort_order');
		}
																		
		$this->view->pick('total/credit');
		$this->_commonAction();
