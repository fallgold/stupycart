...
class TaxController extends \Stupycart\Admin\Controllers\ControllerBase { 
	private $error = array();
	...
	public function indexAction() { 
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('tax', $this->request->getPostE());
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('total/tax', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('total/tax', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/total', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('tax_status')) {
			$this->data['tax_status'] = $this->request->getPostE('tax_status');
		} else {
			$this->data['tax_status'] = $this->config->get('tax_status');
		}

		if ($this->request->hasPost('tax_sort_order')) {
			$this->data['tax_sort_order'] = $this->request->getPostE('tax_sort_order');
		} else {
			$this->data['tax_sort_order'] = $this->config->get('tax_sort_order');
		}
																				
		$this->view->pick('total/tax');
		$this->_commonAction();
