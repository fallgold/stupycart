...
class LowOrderFeeController extends \Stupycart\Admin\Controllers\ControllerBase { 
	private $error = array(); 
	...
	public function indexAction() { 
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('low_order_fee', $this->request->getPostE());
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('total/low_order_fee', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('total/low_order_fee', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/total', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('low_order_fee_total')) {
			$this->data['low_order_fee_total'] = $this->request->getPostE('low_order_fee_total');
		} else {
			$this->data['low_order_fee_total'] = $this->config->get('low_order_fee_total');
		}
		
		if ($this->request->hasPost('low_order_fee_fee')) {
			$this->data['low_order_fee_fee'] = $this->request->getPostE('low_order_fee_fee');
		} else {
			$this->data['low_order_fee_fee'] = $this->config->get('low_order_fee_fee');
		}

		if ($this->request->hasPost('low_order_fee_tax_class_id')) {
			$this->data['low_order_fee_tax_class_id'] = $this->request->getPostE('low_order_fee_tax_class_id');
		} else {
		...
		if ($this->request->hasPost('low_order_fee_status')) {
			$this->data['low_order_fee_status'] = $this->request->getPostE('low_order_fee_status');
		} else {
			$this->data['low_order_fee_status'] = $this->config->get('low_order_fee_status');
		}

		if ($this->request->hasPost('low_order_fee_sort_order')) {
			$this->data['low_order_fee_sort_order'] = $this->request->getPostE('low_order_fee_sort_order');
		} else {
			$this->data['low_order_fee_sort_order'] = $this->config->get('low_order_fee_sort_order');
		}

		$this->view->pick('total/low_order_fee');
		$this->_commonAction();
