...
class TotalController extends \Stupycart\Admin\Controllers\ControllerBase { 
	...
	public function indexAction() { 
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('total', $this->request->getPostE());
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['warning'])) {
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('total/total', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('total/total', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/total', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('total_status')) {
			$this->data['total_status'] = $this->request->getPostE('total_status');
		} else {
			$this->data['total_status'] = $this->config->get('total_status');
		}

		if ($this->request->hasPost('total_sort_order')) {
			$this->data['total_sort_order'] = $this->request->getPostE('total_sort_order');
		} else {
			$this->data['total_sort_order'] = $this->config->get('total_sort_order');
		}
																		
		$this->view->pick('total/total');
		$this->_commonAction();
