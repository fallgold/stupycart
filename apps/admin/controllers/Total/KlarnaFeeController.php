...
class KlarnaFeeController extends \Stupycart\Admin\Controllers\ControllerBase {
    private $error = array();
    ...
    public function indexAction() {
        ...
        if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			...
			foreach ($this->request->getPostE('klarna_fee') as $klarna_account) {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('total/klarna_fee', 'token=' . $this->session->get('token'), 'SSL'),
            'separator' => ' :: '
        );
                
		$this->data['action'] = $this->url->link('total/klarna_fee', 'token=' . $this->session->get('token'), 'SSL');
        ...
        if ($this->request->hasPost('klarna_fee')) {
            $this->data['klarna_fee'] = $this->request->getPostE('klarna_fee');
        } else {
        ...
        $this->view->pick('total/klarna_fee');
		$this->_commonAction();
