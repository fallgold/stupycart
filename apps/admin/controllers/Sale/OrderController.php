...
class OrderController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array();
  	...
  	public function insertAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
      	  	$this->model_sale_order->addOrder($this->request->getPostE());
			
			$this->session->set('success', $this->language->get('text_success'));
		  
			$url = '';
			
			if ($this->request->hasQuery('filter_order_id')) {
				$url .= '&filter_order_id=' . $this->request->getQueryE('filter_order_id');
			}
			
			if ($this->request->hasQuery('filter_customer')) {
				$url .= '&filter_customer=' . urlencode(html_entity_decode($this->request->getQueryE('filter_customer'), ENT_QUOTES, 'UTF-8'));
			}
			...
			if ($this->request->hasQuery('filter_total')) {
				$url .= '&filter_total=' . $this->request->getQueryE('filter_total');
			}
			...
			if ($this->request->hasQuery('filter_date_modified')) {
				$url .= '&filter_date_modified=' . $this->request->getQueryE('filter_date_modified');
			}
			...
			$this->response->redirect($this->url->link('sale/order', 'token=' . $this->session->get('token') . $url, 'SSL'), true);
		return;
		}
  	...
  	public function updateAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
			$this->model_sale_order->editOrder($this->request->getQueryE('order_id'), $this->request->getPostE());
  	...
  	public function deleteAction() {
    	...
    	if ($this->request->hasPost('selected') && ($this->validateDelete())) {
			foreach ($this->request->getPostE('selected') as $order_id) {
  	...
  	protected function getList() {
		if ($this->request->hasQuery('filter_order_id')) {
			$filter_order_id = $this->request->getQueryE('filter_order_id');
		} else {
		...
		if ($this->request->hasQuery('filter_total')) {
			$filter_total = $this->request->getQueryE('filter_total');
		} else {
			$filter_total = null;
		}
		...
		if ($this->request->hasQuery('filter_date_modified')) {
			$filter_date_modified = $this->request->getQueryE('filter_date_modified');
		} else {
		...
		if ($this->request->hasQuery('page')) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('sale/order', 'token=' . $this->session->get('token') . $url, 'SSL'),
      		'separator' => ' :: '
   		);

		$this->data['invoice'] = $this->url->link('sale/order/invoice', 'token=' . $this->session->get('token'), 'SSL');
		$this->data['insert'] = $this->url->link('sale/order/insert', 'token=' . $this->session->get('token'), 'SSL');
		$this->data['delete'] = $this->url->link('sale/order/delete', 'token=' . $this->session->get('token') . $url, 'SSL');
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/order/info', 'token=' . $this->session->get('token') . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);
			
			if (strtotime($result['date_added']) > strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')) {
				$action[] = array(
					'text' => $this->language->get('text_edit'),
					'href' => $this->url->link('sale/order/update', 'token=' . $this->session->get('token') . '&order_id=' . $result['order_id'] . $url, 'SSL')
				);
			}
			...
			$this->data['orders'][] = array(
				...
				'selected'      => $this->request->hasPost('selected') && in_array($result['order_id'], $this->request->getPostE('selected')),
				'action'        => $action
			);
		}
		...
		$this->data['sort_order'] = $this->url->link('sale/order', 'token=' . $this->session->get('token') . '&sort=o.order_id' . $url, 'SSL');
		$this->data['sort_customer'] = $this->url->link('sale/order', 'token=' . $this->session->get('token') . '&sort=customer' . $url, 'SSL');
		$this->data['sort_status'] = $this->url->link('sale/order', 'token=' . $this->session->get('token') . '&sort=status' . $url, 'SSL');
		$this->data['sort_total'] = $this->url->link('sale/order', 'token=' . $this->session->get('token') . '&sort=o.total' . $url, 'SSL');
		$this->data['sort_date_added'] = $this->url->link('sale/order', 'token=' . $this->session->get('token') . '&sort=o.date_added' . $url, 'SSL');
		$this->data['sort_date_modified'] = $this->url->link('sale/order', 'token=' . $this->session->get('token') . '&sort=o.date_modified' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('sale/order', 'token=' . $this->session->get('token') . $url . '&page={page}', 'SSL');
		...
		$this->view->pick('sale/order_list');
		$this->_commonAction();
  	...
  	public function getForm() {
		...
		if (!$this->request->hasQuery('order_id')) {
			$this->data['action'] = $this->url->link('sale/order/insert', 'token=' . $this->session->get('token') . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('sale/order/update', 'token=' . $this->session->get('token') . '&order_id=' . $this->request->getQueryE('order_id') . $url, 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('sale/order', 'token=' . $this->session->get('token') . $url, 'SSL');

    	if ($this->request->hasQuery('order_id') && ($this->request->getServer('REQUEST_METHOD') != 'POST')) {
      		$order_info = $this->model_sale_order->getOrder($this->request->getQueryE('order_id'));
    	}

		$this->data['token'] = $this->session->get('token');
		
		if ($this->request->hasQuery('order_id')) {
			$this->data['order_id'] = $this->request->getQueryE('order_id');
		} else {
			$this->data['order_id'] = 0;
		}
					
    	if ($this->request->hasPost('store_id')) {
      		$this->data['store_id'] = $this->request->getPostE('store_id');
		...
		if ($this->request->hasPost('customer')) {
			$this->data['customer'] = $this->request->getPostE('customer');
		} elseif (!empty($order_info)) {
		...
		if ($this->request->hasPost('customer_id')) {
			$this->data['customer_id'] = $this->request->getPostE('customer_id');
		} elseif (!empty($order_info)) {
		...
		if ($this->request->hasPost('affiliate_id')) {
      		$this->data['affiliate_id'] = $this->request->getPostE('affiliate_id');
    	} elseif (!empty($order_info)) { 
		...
		if ($this->request->hasPost('affiliate')) {
      		$this->data['affiliate'] = $this->request->getPostE('affiliate');
    	} elseif (!empty($order_info)) { 
		...
		if ($this->request->hasPost('order_status_id')) {
      		$this->data['order_status_id'] = $this->request->getPostE('order_status_id');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('comment')) {
      		$this->data['comment'] = $this->request->getPostE('comment');
    	} elseif (!empty($order_info)) { 
		...
		if ($this->request->hasPost('customer_id')) {
			$this->data['addresses'] = $this->model_sale_customer->getAddresses($this->request->getPostE('customer_id'));
		} elseif (!empty($order_info)) {
    	...
    	if ($this->request->hasPost('payment_firstname')) {
      		$this->data['payment_firstname'] = $this->request->getPostE('payment_firstname');
		} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_lastname')) {
      		$this->data['payment_lastname'] = $this->request->getPostE('payment_lastname');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_company')) {
      		$this->data['payment_company'] = $this->request->getPostE('payment_company');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_company_id')) {
      		$this->data['payment_company_id'] = $this->request->getPostE('payment_company_id');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_tax_id')) {
      		$this->data['payment_tax_id'] = $this->request->getPostE('payment_tax_id');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_address_1')) {
      		$this->data['payment_address_1'] = $this->request->getPostE('payment_address_1');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_address_2')) {
      		$this->data['payment_address_2'] = $this->request->getPostE('payment_address_2');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_city')) {
      		$this->data['payment_city'] = $this->request->getPostE('payment_city');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_postcode')) {
      		$this->data['payment_postcode'] = $this->request->getPostE('payment_postcode');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_country_id')) {
      		$this->data['payment_country_id'] = $this->request->getPostE('payment_country_id');
    	} elseif (!empty($order_info)) { 
		...
		if ($this->request->hasPost('payment_zone_id')) {
      		$this->data['payment_zone_id'] = $this->request->getPostE('payment_zone_id');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_method')) {
      		$this->data['payment_method'] = $this->request->getPostE('payment_method');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('payment_code')) {
      		$this->data['payment_code'] = $this->request->getPostE('payment_code');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_firstname')) {
      		$this->data['shipping_firstname'] = $this->request->getPostE('shipping_firstname');
		} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_lastname')) {
      		$this->data['shipping_lastname'] = $this->request->getPostE('shipping_lastname');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_company')) {
      		$this->data['shipping_company'] = $this->request->getPostE('shipping_company');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_address_1')) {
      		$this->data['shipping_address_1'] = $this->request->getPostE('shipping_address_1');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_address_2')) {
      		$this->data['shipping_address_2'] = $this->request->getPostE('shipping_address_2');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_city')) {
      		$this->data['shipping_city'] = $this->request->getPostE('shipping_city');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_postcode')) {
      		$this->data['shipping_postcode'] = $this->request->getPostE('shipping_postcode');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_country_id')) {
      		$this->data['shipping_country_id'] = $this->request->getPostE('shipping_country_id');
    	} elseif (!empty($order_info)) { 
		...
		if ($this->request->hasPost('shipping_zone_id')) {
      		$this->data['shipping_zone_id'] = $this->request->getPostE('shipping_zone_id');
    	} elseif (!empty($order_info)) { 
    	...
    	if ($this->request->hasPost('shipping_code')) {
      		$this->data['shipping_code'] = $this->request->getPostE('shipping_code');
    	} elseif (!empty($order_info)) { 
		...
		if ($this->request->hasPost('order_product')) {
			$order_products = $this->request->getPostE('order_product');
		} elseif ($this->request->hasQuery('order_id')) {
			$order_products = $this->model_sale_order->getOrderProducts($this->request->getQueryE('order_id'));			
		} else {
		...
		foreach ($order_products as $order_product) {
			if (isset($order_product['order_option'])) {
				...
				$order_option = $this->model_sale_order->getOrderOptions($this->request->getQueryE('order_id'), $order_product['order_product_id']);
			} else {
				$order_option = array();
			}
			...
			if (isset($order_product['order_download'])) {
				...
				$order_download = $this->model_sale_order->getOrderDownloads($this->request->getQueryE('order_id'), $order_product['order_product_id']);
			} else {
		...
		if ($this->request->hasPost('order_voucher')) {
			$this->data['order_vouchers'] = $this->request->getPostE('order_voucher');
		} elseif ($this->request->hasQuery('order_id')) {
			$this->data['order_vouchers'] = $this->model_sale_order->getOrderVouchers($this->request->getQueryE('order_id'));			
		} else {
		...
		if ($this->request->hasPost('order_total')) {
      		$this->data['order_totals'] = $this->request->getPostE('order_total');
    	} elseif ($this->request->hasQuery('order_id')) { 
			$this->data['order_totals'] = $this->model_sale_order->getOrderTotals($this->request->getQueryE('order_id'));
		} else {
      		$this->data['order_totals'] = array();
    	}	
		
		$this->view->pick('sale/order_form');
		$this->_commonAction();
  	...
  	protected function validateForm() {
    	...
    	if ((utf8_strlen($this->request->getPostE('payment_firstname')) < 1) || (utf8_strlen($this->request->getPostE('payment_firstname')) > 32)) {
      		$this->error['payment_firstname'] = $this->language->get('error_firstname');
    	}

    	if ((utf8_strlen($this->request->getPostE('payment_lastname')) < 1) || (utf8_strlen($this->request->getPostE('payment_lastname')) > 32)) {
      		$this->error['payment_lastname'] = $this->language->get('error_lastname');
    	}

    	if ((utf8_strlen($this->request->getPostE('payment_address_1')) < 3) || (utf8_strlen($this->request->getPostE('payment_address_1')) > 128)) {
      		$this->error['payment_address_1'] = $this->language->get('error_address_1');
    	}

    	if ((utf8_strlen($this->request->getPostE('payment_city')) < 3) || (utf8_strlen($this->request->getPostE('payment_city')) > 128)) {
		...
		if ($country_info) {
			if ($country_info['postcode_required'] && (utf8_strlen($this->request->getPostE('payment_postcode')) < 2) || (utf8_strlen($this->request->getPostE('payment_postcode')) > 10)) {
			...
			if ($this->config->get('config_vat') && $this->request->getPostE('payment_tax_id') && (vat_validation($country_info['iso_code_2'], $this->request->getPostE('payment_tax_id')) == 'invalid')) {
    	...
    	if ($this->request->getPostE('payment_method') == '') {
      		$this->error['payment_zone'] = $this->language->get('error_zone');
    	}			
		...
		if ($shipping) {
			if ((utf8_strlen($this->request->getPostE('shipping_firstname')) < 1) || (utf8_strlen($this->request->getPostE('shipping_firstname')) > 32)) {
				$this->error['shipping_firstname'] = $this->language->get('error_firstname');
			}
	
			if ((utf8_strlen($this->request->getPostE('shipping_lastname')) < 1) || (utf8_strlen($this->request->getPostE('shipping_lastname')) > 32)) {
				$this->error['shipping_lastname'] = $this->language->get('error_lastname');
			}
			
			if ((utf8_strlen($this->request->getPostE('shipping_address_1')) < 3) || (utf8_strlen($this->request->getPostE('shipping_address_1')) > 128)) {
				$this->error['shipping_address_1'] = $this->language->get('error_address_1');
			}
	
			if ((utf8_strlen($this->request->getPostE('shipping_city')) < 3) || (utf8_strlen($this->request->getPostE('shipping_city')) > 128)) {
	...
	public function infoAction() {
		...
		if ($this->request->hasQuery('order_id')) {
			$order_id = $this->request->getQueryE('order_id');
		} else {
			$order_id = 0;
		}
		...
		if ($order_info) {
			...
			$this->data['invoice'] = $this->url->link('sale/order/invoice', 'token=' . $this->session->get('token') . '&order_id=' . (int)$this->request->getQueryE('order_id'), 'SSL');
			$this->data['cancel'] = $this->url->link('sale/order', 'token=' . $this->session->get('token') . $url, 'SSL');
			...
			if ($order_info['customer_id']) {
				$this->data['customer'] = $this->url->link('sale/customer/update', 'token=' . $this->session->get('token') . '&customer_id=' . $order_info['customer_id'], 'SSL');
			} else {
				$this->data['customer'] = '';
			}
			...
			$this->data['credit_total'] = $this->model_sale_customer->getTotalTransactionsByOrderId($this->request->getQueryE('order_id')); 
			
			$this->data['reward'] = $order_info['reward'];
						
			$this->data['reward_total'] = $this->model_sale_customer->getTotalCustomerRewardsByOrderId($this->request->getQueryE('order_id'));
			...
			if ($order_info['affiliate_id']) {
				$this->data['affiliate'] = $this->url->link('sale/affiliate/update', 'token=' . $this->session->get('token') . '&affiliate_id=' . $order_info['affiliate_id'], 'SSL');
			} else {
				$this->data['affiliate'] = '';
			}
			...
			$this->data['commission_total'] = $this->model_sale_affiliate->getTotalTransactionsByOrderId($this->request->getQueryE('order_id')); 
			...
			$products = $this->model_sale_order->getOrderProducts($this->request->getQueryE('order_id'));

			foreach ($products as $product) {
				$option_data = array();

				$options = $this->model_sale_order->getOrderOptions($this->request->getQueryE('order_id'), $product['order_product_id']);
				...
				foreach ($options as $option) {
					if ($option['type'] != 'file') {
						...
						$option_data[] = array(
							...
							'type'  => $option['type'],
							'href'  => $this->url->link('sale/order/download', 'token=' . $this->session->get('token') . '&order_id=' . $this->request->getQueryE('order_id') . '&order_option_id=' . $option['order_option_id'], 'SSL')
						);						
					}
				}
				...
				$this->data['products'][] = array(
					...
					'href'     		   => $this->url->link('catalog/product/update', 'token=' . $this->session->get('token') . '&product_id=' . $product['product_id'], 'SSL')
				);
			}
		
			$this->data['vouchers'] = array();	
			
			$vouchers = $this->model_sale_order->getOrderVouchers($this->request->getQueryE('order_id'));
			 
			foreach ($vouchers as $voucher) {
				$this->data['vouchers'][] = array(
					'description' => $voucher['description'],
					'amount'      => $this->currency->format($voucher['amount'], $order_info['currency_code'], $order_info['currency_value']),
					'href'        => $this->url->link('sale/voucher/update', 'token=' . $this->session->get('token') . '&voucher_id=' . $voucher['voucher_id'], 'SSL')
				);
			}
		
			$this->data['totals'] = $this->model_sale_order->getOrderTotals($this->request->getQueryE('order_id'));

			$this->data['downloads'] = array();

			foreach ($products as $product) {
				$results = $this->model_sale_order->getOrderDownloads($this->request->getQueryE('order_id'), $product['order_product_id']);
			...
			// Fraud
			$this->model_sale_fraud = new \Stupycart\Common\Models\Admin\Sale\Fraud();
			...
			$this->view->pick('sale/order_info');
		$this->_commonAction();
	...
	public function createInvoiceNo() {
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			$invoice_no = $this->model_sale_order->createInvoiceNo($this->request->getQueryE('order_id'));
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['customer_id']) {
				...
				$credit_total = $this->model_sale_customer->getTotalTransactionsByOrderId($this->request->getQueryE('order_id'));
				
				if (!$credit_total) {
					$this->model_sale_customer->addTransaction($order_info['customer_id'], $this->language->get('text_order_id') . ' #' . $this->request->getQueryE('order_id'), $order_info['total'], $this->request->getQueryE('order_id'));
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['customer_id']) {
				...
				$this->model_sale_customer->deleteTransaction($this->request->getQueryE('order_id'));
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['customer_id']) {
				...
				$reward_total = $this->model_sale_customer->getTotalCustomerRewardsByOrderId($this->request->getQueryE('order_id'));
				
				if (!$reward_total) {
					$this->model_sale_customer->addReward($order_info['customer_id'], $this->language->get('text_order_id') . ' #' . $this->request->getQueryE('order_id'), $order_info['reward'], $this->request->getQueryE('order_id'));
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['customer_id']) {
				...
				$this->model_sale_customer->deleteReward($this->request->getQueryE('order_id'));
		...
		$json = array();
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['affiliate_id']) {
				...
				$affiliate_total = $this->model_sale_affiliate->getTotalTransactionsByOrderId($this->request->getQueryE('order_id'));
				
				if (!$affiliate_total) {
					$this->model_sale_affiliate->addTransaction($order_info['affiliate_id'], $this->language->get('text_order_id') . ' #' . $this->request->getQueryE('order_id'), $order_info['commission'], $this->request->getQueryE('order_id'));
		...
		$json = array(); 
     	...
     	if (!$this->user->hasPermission('modify', 'sale/order')) {
			...
			if ($order_info && $order_info['affiliate_id']) {
				...
				$this->model_sale_affiliate->deleteTransaction($this->request->getQueryE('order_id'));
	...
	public function historyAction() {
		...
		if ($this->request->getServer('REQUEST_METHOD') == 'POST') {
			...
			if (!$this->data['error']) { 
				$this->model_sale_order->addOrderHistory($this->request->getQueryE('order_id'), $this->request->getPostE());
		...
		$results = $this->model_sale_order->getOrderHistories($this->request->getQueryE('order_id'), ($page - 1) * 10, 10);
		...
		$history_total = $this->model_sale_order->getTotalOrderHistories($this->request->getQueryE('order_id'));
		...
		$pagination->limit = 10; 
		$pagination->text = $this->language->get('text_pagination');
		$pagination->url = $this->url->link('sale/order/history', 'token=' . $this->session->get('token') . '&order_id=' . $this->request->getQueryE('order_id') . '&page={page}', 'SSL');
			
		$this->data['pagination'] = $pagination->render();
		
		$this->view->pick('sale/order_history');		
	...
	public function downloadAction() {
		...
		if ($this->request->hasQuery('order_option_id')) {
			$order_option_id = $this->request->getQueryE('order_option_id');
		} else {
			$order_option_id = 0;
		}
		
		$option_info = $this->model_sale_order->getOrderOption($this->request->getQueryE('order_id'), $order_option_id);
	...
	public function uploadAction() {
  	...
  	public function invoiceAction() {
		...
		if ($this->request->hasPost('selected')) {
			$orders = $this->request->getPostE('selected');
		} elseif ($this->request->hasQuery('order_id')) {
			$orders[] = $this->request->getQueryE('order_id');
		}
		...
		$this->view->pick('sale/order_invoice');
