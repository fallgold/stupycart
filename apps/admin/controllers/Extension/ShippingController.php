...
class ShippingController extends \Stupycart\Admin\Controllers\ControllerBase {
	public function indexAction() {
			...
			foreach ($files as $file) {
				...
				if (!in_array($extension, $extensions)) {
					$action[] = array(
						...
						'href' => $this->url->link('extension/shipping/install', 'token=' . $this->session->get('token') . '&extension=' . $extension, 'SSL')
					);
				} else {
					$action[] = array(
						'text' => $this->language->get('text_edit'),
						'href' => $this->url->link('shipping/' . $extension . '', 'token=' . $this->session->get('token'), 'SSL')
					);
								
					$action[] = array(
						'text' => $this->language->get('text_uninstall'),
						'href' => $this->url->link('extension/shipping/uninstall', 'token=' . $this->session->get('token') . '&extension=' . $extension, 'SSL')
					);
				}
		...
		$this->view->pick('extension/shipping');
		$this->_commonAction();
	...
	public function installAction() {
		...
		return;
			...
			$this->model_setting_extension->install('shipping', $this->request->getQueryE('extension'));

			$this->model_user_user_group = new \Stupycart\Common\Models\Admin\User\UserGroup();
		
			$this->model_user_user_group->addPermission($this->user->getId(), 'access', 'shipping/' . $this->request->getQueryE('extension'));
			$this->model_user_user_group->addPermission($this->user->getId(), 'modify', 'shipping/' . $this->request->getQueryE('extension'));

			require_once(DIR_APPLICATION . 'controller/shipping/' . $this->request->getQueryE('extension') . '.php');
			
			$class = 'ControllerShipping' . str_replace('_', '', $this->request->getQueryE('extension'));
	...
	public function uninstallAction() {
		...
		return;
			...
			$this->model_setting_extension->uninstall('shipping', $this->request->getQueryE('extension'));
