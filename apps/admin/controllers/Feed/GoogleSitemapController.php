...
class GoogleSitemapController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('google_sitemap', $this->request->getPostE());				
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('feed/google_sitemap', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
				
		$this->data['action'] = $this->url->link('feed/google_sitemap', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/feed', 'token=' . $this->session->get('token'), 'SSL');
		
		if ($this->request->hasPost('google_sitemap_status')) {
			$this->data['google_sitemap_status'] = $this->request->getPostE('google_sitemap_status');
		} else {
		...
		$this->view->pick('feed/google_sitemap');
		$this->_commonAction();
