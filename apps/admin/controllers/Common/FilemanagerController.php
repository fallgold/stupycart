...
class FileManagerController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array();
	...
	public function indexAction() {
		...
		if ($this->request->hasQuery('field')) {
			$this->data['field'] = $this->request->getQueryE('field');
		} else {
			$this->data['field'] = '';
		}
		
		if ($this->request->hasQuery('CKEditorFuncNum')) {
			$this->data['fckeditor'] = $this->request->getQueryE('CKEditorFuncNum');
		} else {
			$this->data['fckeditor'] = false;
		}
		
		$this->view->pick('common/filemanager');
	...
	public function imageAction() {
		...
		if ($this->request->hasQuery('image')) {
			$this->response->setContent($this->model_tool_image->resize(html_entity_decode($this->request->getQueryE('image'), ENT_QUOTES, 'UTF-8'), 100, 100));
	...
	public function directoryAction() {	
		...
		if ($this->request->hasPost('directory')) {
			$directories = glob(rtrim(DIR_IMAGE . 'data/' . str_replace('../', '', $this->request->getPostE('directory')), '/') . '/*', GLOB_ONLYDIR); 
	...
	public function filesAction() {
		...
		if (!(!$this->request->getPostE('directory'))) {
			$directory = DIR_IMAGE . 'data/' . str_replace('../', '', $this->request->getPostE('directory'));
		} else {
	...
	public function createAction() {
		...
		if ($this->request->hasPost('directory')) {
			if ($this->request->hasPost('name') || $this->request->getPostE('name')) {
				$directory = rtrim(DIR_IMAGE . 'data/' . str_replace('../', '', $this->request->getPostE('directory')), '/');							   
				
				if (!is_dir($directory)) {
					$json['error'] = $this->language->get('error_directory');
				}
				
				if (file_exists($directory . '/' . str_replace('../', '', $this->request->getPostE('name')))) {
		...
		if (!isset($json['error'])) {	
			mkdir($directory . '/' . str_replace('../', '', $this->request->getPostE('name')), 0777);
	...
	public function deleteAction() {
		...
		if ($this->request->hasPost('path')) {
			$path = rtrim(DIR_IMAGE . 'data/' . str_replace('../', '', html_entity_decode($this->request->getPostE('path'), ENT_QUOTES, 'UTF-8')), '/');
	...
	public function moveAction() {
		...
		if ($this->request->hasPost('from') && $this->request->hasPost('to')) {
			$from = rtrim(DIR_IMAGE . 'data/' . str_replace('../', '', html_entity_decode($this->request->getPostE('from'), ENT_QUOTES, 'UTF-8')), '/');
			...
			$to = rtrim(DIR_IMAGE . 'data/' . str_replace('../', '', html_entity_decode($this->request->getPostE('to'), ENT_QUOTES, 'UTF-8')), '/');
	...
	public function copyAction() {
		...
		if ($this->request->hasPost('path') && $this->request->hasPost('name')) {
			if ((utf8_strlen($this->request->getPostE('name')) < 3) || (utf8_strlen($this->request->getPostE('name')) > 255)) {
				$json['error'] = $this->language->get('error_filename');
			}
				
			$old_name = rtrim(DIR_IMAGE . 'data/' . str_replace('../', '', html_entity_decode($this->request->getPostE('path'), ENT_QUOTES, 'UTF-8')), '/');
			...
			$new_name = dirname($old_name) . '/' . str_replace('../', '', html_entity_decode($this->request->getPostE('name'), ENT_QUOTES, 'UTF-8') . $ext);
	...
	public function foldersAction() {
		$this->response->setContent($this->recursiveFolders(DIR_IMAGE . 'data/'));
		return $this->response;	
	}
	...
	public function uploadAction() {
		...
		if ($this->request->hasPost('directory')) {
			if (isset($this->request->files['image']) && $_FILES['image']['tmp_name']) {
