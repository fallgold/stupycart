...
class ForgottenController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array();
	...
	public function indexAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			...
			$this->model_user_user->editCode($this->request->getPostE('email'), $code);
			...
			$message .= sprintf($this->language->get('text_ip'), $this->request->getServer('REMOTE_ADDR')) . "\n\n";
		...
		$this->view->pick('common/forgotten');
		$this->_commonAction();
	...
	protected function validate() {
		...
		} elseif (!$this->model_user_user->getTotalUsersByEmail($this->request->getPostE('email'))) {
			$this->error['warning'] = $this->language->get('error_email');
		}
