...
class HomeController extends \Stupycart\Admin\Controllers\ControllerBase {   
	public function indexAction() {
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('sale/order/info', 'token=' . $this->session->get('token') . '&order_id=' . $result['order_id'], 'SSL')
			);
	...
	public function chartAction() {
		...
		if ($this->request->hasQuery('range')) {
			$range = $this->request->getQueryE('range');
		} else {
			$range = 'month';
		}
		...
		switch ($range) {
			case 'day':
				for ($i = 0; $i < 24; $i++) {
					$query = $this->ocdb->db_query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '" . (int)$this->config->get('config_complete_status_id') . "' AND (DATE(date_added) = DATE(NOW()) AND HOUR(date_added) = '" . (int)$i . "') GROUP BY HOUR(date_added) ORDER BY date_added ASC");
					...
					$query = $this->ocdb->db_query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE DATE(date_added) = DATE(NOW()) AND HOUR(date_added) = '" . (int)$i . "' GROUP BY HOUR(date_added) ORDER BY date_added ASC");
			...
			case 'week':
				...
				for ($i = 0; $i < 7; $i++) {
					...
					$query = $this->ocdb->db_query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '" . (int)$this->config->get('config_complete_status_id') . "' AND DATE(date_added) = '" . $this->ocdb->db_escape($date) . "' GROUP BY DATE(date_added)");
					...
					$query = $this->ocdb->db_query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "customer` WHERE DATE(date_added) = '" . $this->ocdb->db_escape($date) . "' GROUP BY DATE(date_added)");
			...
			case 'month':
				for ($i = 1; $i <= date('t'); $i++) {
					...
					$query = $this->ocdb->db_query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '" . (int)$this->config->get('config_complete_status_id') . "' AND (DATE(date_added) = '" . $this->ocdb->db_escape($date) . "') GROUP BY DAY(date_added)");
					...
					$query = $this->ocdb->db_query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE DATE(date_added) = '" . $this->ocdb->db_escape($date) . "' GROUP BY DAY(date_added)");
			...
			case 'year':
				for ($i = 1; $i <= 12; $i++) {
					$query = $this->ocdb->db_query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE order_status_id > '" . (int)$this->config->get('config_complete_status_id') . "' AND YEAR(date_added) = '" . date('Y') . "' AND MONTH(date_added) = '" . $i . "' GROUP BY MONTH(date_added)");
					...
					$query = $this->ocdb->db_query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "customer WHERE YEAR(date_added) = '" . date('Y') . "' AND MONTH(date_added) = '" . $i . "' GROUP BY MONTH(date_added)");
	...
	public function loginAction() {
		...
		if ($this->request->hasQuery('route')) {
			...
			if (!in_array($route, $ignore) && (!$this->request->hasQuery('token') || !$this->session->has('token') || ($this->request->getQueryE('token') != $this->session->get('token')))) {
				return $this->forward('common/login');
			}
		} else {
			if (!$this->request->hasQuery('token') || !$this->session->has('token') || ($this->request->getQueryE('token') != $this->session->get('token'))) {
				return $this->forward('common/login');
			}
		}
	}
