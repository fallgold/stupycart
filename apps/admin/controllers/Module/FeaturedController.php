...
class FeaturedController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {   
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {			
			$this->model_setting_setting->editSetting('featured', $this->request->getPostE());		
		...
		if (isset($this->error['image'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/featured', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/featured', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->get('token'), 'SSL');

		$this->data['token'] = $this->session->get('token');

		if ($this->request->hasPost('featured_product')) {
			$this->data['featured_product'] = $this->request->getPostE('featured_product');
		} else {
		...
		if ($this->request->hasPost('featured_product')) {
			$products = explode(',', $this->request->getPostE('featured_product'));
		} else {		
		...
		if ($this->request->hasPost('featured_module')) {
			$this->data['modules'] = $this->request->getPostE('featured_module');
		...
		$this->view->pick('module/featured');
		$this->_commonAction();
	...
	protected function validate() {
		...
		if ($this->request->hasPost('featured_module')) {
			foreach ($this->request->getPostE('featured_module') as $key => $value) {
