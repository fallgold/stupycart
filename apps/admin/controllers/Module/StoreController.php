...
class StoreController extends \Stupycart\Admin\Controllers\ControllerBase {
	...
	public function indexAction() {   
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('store', $this->request->getPostE());		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/store', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/store', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('store_admin')) {
			$this->data['store_admin'] = $this->request->getPostE('store_admin');
		} else {
		...
		if ($this->request->hasPost('store_module')) {
			$this->data['modules'] = $this->request->getPostE('store_module');
