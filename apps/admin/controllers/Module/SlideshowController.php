...
class SlideshowController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {   
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('slideshow', $this->request->getPostE());		
		...
		if (isset($this->error['dimension'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/slideshow', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/slideshow', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['modules'] = array();
		
		if ($this->request->hasPost('slideshow_module')) {
			$this->data['modules'] = $this->request->getPostE('slideshow_module');
	...
	protected function validate() {
		...
		if ($this->request->hasPost('slideshow_module')) {
			foreach ($this->request->getPostE('slideshow_module') as $key => $value) {
