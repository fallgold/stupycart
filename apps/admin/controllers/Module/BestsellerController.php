...
class BestSellerController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {   
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('bestseller', $this->request->getPostE());		
		...
		if (isset($this->error['image'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/bestseller', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('module/bestseller', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/module', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['modules'] = array();
		
		if ($this->request->hasPost('bestseller_module')) {
			$this->data['modules'] = $this->request->getPostE('bestseller_module');
		...
		$this->view->pick('module/bestseller');
		$this->_commonAction();
	...
	protected function validate() {
		...
		if ($this->request->hasPost('bestseller_module')) {
			foreach ($this->request->getPostE('bestseller_module') as $key => $value) {
