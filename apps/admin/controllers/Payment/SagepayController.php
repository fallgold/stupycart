...
class SagepayController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('sagepay', $this->request->getPostE());				
 		...
 		if (isset($this->error['password'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/sagepay', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
				
		$this->data['action'] = $this->url->link('payment/sagepay', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->get('token'), 'SSL');
		
		if ($this->request->hasPost('sagepay_vendor')) {
			$this->data['sagepay_vendor'] = $this->request->getPostE('sagepay_vendor');
		} else {
			$this->data['sagepay_vendor'] = $this->config->get('sagepay_vendor');
		}
		
		if ($this->request->hasPost('sagepay_password')) {
			$this->data['sagepay_password'] = $this->request->getPostE('sagepay_password');
		} else {
			$this->data['sagepay_password'] = $this->config->get('sagepay_password');
		}

		if ($this->request->hasPost('sagepay_test')) {
			$this->data['sagepay_test'] = $this->request->getPostE('sagepay_test');
		} else {
			$this->data['sagepay_test'] = $this->config->get('sagepay_test');
		}
		
		if ($this->request->hasPost('sagepay_transaction')) {
			$this->data['sagepay_transaction'] = $this->request->getPostE('sagepay_transaction');
		} else {
			$this->data['sagepay_transaction'] = $this->config->get('sagepay_transaction');
		}
		
		if ($this->request->hasPost('sagepay_total')) {
			$this->data['sagepay_total'] = $this->request->getPostE('sagepay_total');
		} else {
			$this->data['sagepay_total'] = $this->config->get('sagepay_total'); 
		} 
				
		if ($this->request->hasPost('sagepay_order_status_id')) {
			$this->data['sagepay_order_status_id'] = $this->request->getPostE('sagepay_order_status_id');
		} else {
		...
		if ($this->request->hasPost('sagepay_geo_zone_id')) {
			$this->data['sagepay_geo_zone_id'] = $this->request->getPostE('sagepay_geo_zone_id');
		} else {
		...
		if ($this->request->hasPost('sagepay_status')) {
			$this->data['sagepay_status'] = $this->request->getPostE('sagepay_status');
		} else {
			$this->data['sagepay_status'] = $this->config->get('sagepay_status');
		}
		
		if ($this->request->hasPost('sagepay_sort_order')) {
			$this->data['sagepay_sort_order'] = $this->request->getPostE('sagepay_sort_order');
		} else {
			$this->data['sagepay_sort_order'] = $this->config->get('sagepay_sort_order');
		}

		$this->view->pick('payment/sagepay');
		$this->_commonAction();
	...
	protected function validate() {
		...
		if (!$this->request->getPostE('sagepay_vendor')) {
			$this->error['vendor'] = $this->language->get('error_vendor');
		}

		if (!$this->request->getPostE('sagepay_password')) {
