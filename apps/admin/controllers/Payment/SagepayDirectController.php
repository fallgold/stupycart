...
class SagepayDirectController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('sagepay_direct', $this->request->getPostE());				
 		...
 		if (isset($this->error['vendor'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/sagepay_direct', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
				
		$this->data['action'] = $this->url->link('payment/sagepay_direct', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->get('token'), 'SSL');
		
		if ($this->request->hasPost('sagepay_direct_vendor')) {
			$this->data['sagepay_direct_vendor'] = $this->request->getPostE('sagepay_direct_vendor');
		} else {
			$this->data['sagepay_direct_vendor'] = $this->config->get('sagepay_direct_vendor');
		}
		
		if ($this->request->hasPost('sagepay_direct_password')) {
			$this->data['sagepay_direct_password'] = $this->request->getPostE('sagepay_direct_password');
		} else {
			$this->data['sagepay_direct_password'] = $this->config->get('sagepay_direct_password');
		}


		if ($this->request->hasPost('sagepay_direct_test')) {
			$this->data['sagepay_direct_test'] = $this->request->getPostE('sagepay_direct_test');
		} else {
			$this->data['sagepay_direct_test'] = $this->config->get('sagepay_direct_test');
		}
		
		if ($this->request->hasPost('sagepay_direct_transaction')) {
			$this->data['sagepay_direct_transaction'] = $this->request->getPostE('sagepay_direct_transaction');
		} else {
			$this->data['sagepay_direct_transaction'] = $this->config->get('sagepay_direct_transaction');
		}
		
		if ($this->request->hasPost('sagepay_direct_total')) {
			$this->data['sagepay_direct_total'] = $this->request->getPostE('sagepay_direct_total');
		} else {
			$this->data['sagepay_direct_total'] = $this->config->get('sagepay_direct_total'); 
		} 
				
		if ($this->request->hasPost('sagepay_direct_order_status_id')) {
			$this->data['sagepay_direct_order_status_id'] = $this->request->getPostE('sagepay_direct_order_status_id');
		} else {
		...
		if ($this->request->hasPost('sagepay_direct_geo_zone_id')) {
			$this->data['sagepay_direct_geo_zone_id'] = $this->request->getPostE('sagepay_direct_geo_zone_id');
		} else {
		...
		if ($this->request->hasPost('sagepay_direct_status')) {
			$this->data['sagepay_direct_status'] = $this->request->getPostE('sagepay_direct_status');
		} else {
			$this->data['sagepay_direct_status'] = $this->config->get('sagepay_direct_status');
		}
		
		if ($this->request->hasPost('sagepay_direct_sort_order')) {
			$this->data['sagepay_direct_sort_order'] = $this->request->getPostE('sagepay_direct_sort_order');
		} else {
			$this->data['sagepay_direct_sort_order'] = $this->config->get('sagepay_direct_sort_order');
		}

		$this->view->pick('payment/sagepay_direct');
		$this->_commonAction();
	...
	protected function validate() {
		...
		if (!$this->request->getPostE('sagepay_direct_vendor')) {
