...
class PerpetualPaymentsController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('perpetual_payments', $this->request->getPostE());				
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/perpetual_payments', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
				
		$this->data['action'] = $this->url->link('payment/perpetual_payments', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('perpetual_payments_auth_id')) {
			$this->data['perpetual_payments_auth_id'] = $this->request->getPostE('perpetual_payments_auth_id');
		} else {
			$this->data['perpetual_payments_auth_id'] = $this->config->get('perpetual_payments_auth_id');
		}
		
		if ($this->request->hasPost('perpetual_payments_auth_pass')) {
			$this->data['perpetual_payments_auth_pass'] = $this->request->getPostE('perpetual_payments_auth_pass');
		} else {
			$this->data['perpetual_payments_auth_pass'] = $this->config->get('perpetual_payments_auth_pass');
		}
		
		if ($this->request->hasPost('perpetual_payments_test')) {
			$this->data['perpetual_payments_test'] = $this->request->getPostE('perpetual_payments_test');
		} else {
			$this->data['perpetual_payments_test'] = $this->config->get('perpetual_payments_test');
		}
		
		if ($this->request->hasPost('perpetual_payments_total')) {
			$this->data['perpetual_payments_total'] = $this->request->getPostE('perpetual_payments_total');
		} else {
			$this->data['perpetual_payments_total'] = $this->config->get('perpetual_payments_total'); 
		} 
				
		if ($this->request->hasPost('perpetual_payments_order_status_id')) {
			$this->data['perpetual_payments_order_status_id'] = $this->request->getPostE('perpetual_payments_order_status_id');
		} else {
		...
		if ($this->request->hasPost('perpetual_payments_geo_zone_id')) {
			$this->data['perpetual_payments_geo_zone_id'] = $this->request->getPostE('perpetual_payments_geo_zone_id');
		} else {
		...
		if ($this->request->hasPost('perpetual_payments_status')) {
			$this->data['perpetual_payments_status'] = $this->request->getPostE('perpetual_payments_status');
		} else {
			$this->data['perpetual_payments_status'] = $this->config->get('perpetual_payments_status');
		}
		
		if ($this->request->hasPost('perpetual_payments_sort_order')) {
			$this->data['perpetual_payments_sort_order'] = $this->request->getPostE('perpetual_payments_sort_order');
		} else {
			$this->data['perpetual_payments_sort_order'] = $this->config->get('perpetual_payments_sort_order');
		}

		$this->view->pick('payment/perpetual_payments');
		$this->_commonAction();
	...
	protected function validate() {
		...
		if (!$this->request->getPostE('perpetual_payments_auth_id')) {
			$this->error['auth_id'] = $this->language->get('error_auth_id');
		}

		if (!$this->request->getPostE('perpetual_payments_auth_pass')) {
