...
class KlarnaInvoiceController extends \Stupycart\Admin\Controllers\ControllerBase {
    private $error = array();
    ...
    public function indexAction() {
        ...
        if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			...
			foreach ($this->request->getPostE('klarna_invoice') as $klarna_invoice) {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('payment/klarna_invoice', 'token=' . $this->session->get('token'), 'SSL'),
            'separator' => ' :: '
        );
        ...
        $this->data['action'] = $this->url->link('payment/klarna_invoice', 'token=' . $this->session->get('token'), 'SSL');
		...
		if ($this->request->hasPost('klarna_invoice')) {
			$this->data['klarna_invoice'] = $this->request->getPostE('klarna_invoice');
		} else {
        ...
        $this->data['clear'] = $this->url->link('payment/klarna_invoice/clear', 'token=' . $this->session->get('token'), 'SSL'); 
    ...
    public function clearAction() {
        ...
        $this->response->redirect($this->url->link('payment/klarna_invoice', 'token=' . $this->session->get('token'), 'SSL'), true);
		return;
    }    
}
