...
class PPProUKController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('pp_pro_uk', $this->request->getPostE());				
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('payment/pp_pro_uk', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
				
		$this->data['action'] = $this->url->link('payment/pp_pro_uk', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/payment', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('pp_pro_uk_vendor')) {
			$this->data['pp_pro_uk_vendor'] = $this->request->getPostE('pp_pro_uk_vendor');
		} else {
			$this->data['pp_pro_uk_vendor'] = $this->config->get('pp_pro_uk_vendor');
		}
		
		if ($this->request->hasPost('pp_pro_uk_user')) {
			$this->data['pp_pro_uk_user'] = $this->request->getPostE('pp_pro_uk_user');
		} else {
			$this->data['pp_pro_uk_user'] = $this->config->get('pp_pro_uk_user');
		}
		
		if ($this->request->hasPost('pp_pro_uk_password')) {
			$this->data['pp_pro_uk_password'] = $this->request->getPostE('pp_pro_uk_password');
		} else {
			$this->data['pp_pro_uk_password'] = $this->config->get('pp_pro_uk_password');
		}
		
		if ($this->request->hasPost('pp_pro_uk_partner')) {
			$this->data['pp_pro_uk_partner'] = $this->request->getPostE('pp_pro_uk_partner');
		...
		if ($this->request->hasPost('pp_pro_uk_test')) {
			$this->data['pp_pro_uk_test'] = $this->request->getPostE('pp_pro_uk_test');
		} else {
			$this->data['pp_pro_uk_test'] = $this->config->get('pp_pro_uk_test');
		}
		
		if ($this->request->hasPost('pp_pro_uk_method')) {
			$this->data['pp_pro_uk_transaction'] = $this->request->getPostE('pp_pro_uk_transaction');
		} else {
			$this->data['pp_pro_uk_transaction'] = $this->config->get('pp_pro_uk_transaction');
		}
		
		if ($this->request->hasPost('pp_pro_uk_total')) {
			$this->data['pp_pro_uk_total'] = $this->request->getPostE('pp_pro_uk_total');
		} else {
			$this->data['pp_pro_uk_total'] = $this->config->get('pp_pro_uk_total'); 
		} 
				
		if ($this->request->hasPost('pp_pro_uk_order_status_id')) {
			$this->data['pp_pro_uk_order_status_id'] = $this->request->getPostE('pp_pro_uk_order_status_id');
		} else {
		...
		if ($this->request->hasPost('pp_pro_uk_geo_zone_id')) {
			$this->data['pp_pro_uk_geo_zone_id'] = $this->request->getPostE('pp_pro_uk_geo_zone_id');
		} else {
		...
		if ($this->request->hasPost('pp_pro_uk_status')) {
			$this->data['pp_pro_uk_status'] = $this->request->getPostE('pp_pro_uk_status');
		} else {
			$this->data['pp_pro_uk_status'] = $this->config->get('pp_pro_uk_status');
		}
		
		if ($this->request->hasPost('pp_pro_uk_sort_order')) {
			$this->data['pp_pro_uk_sort_order'] = $this->request->getPostE('pp_pro_uk_sort_order');
		} else {
			$this->data['pp_pro_uk_sort_order'] = $this->config->get('pp_pro_uk_sort_order');
		}

		$this->view->pick('payment/pp_pro_uk');
		$this->_commonAction();
	...
	protected function validate() {
		...
		if (!$this->request->getPostE('pp_pro_uk_vendor')) {
			$this->error['vendor'] = $this->language->get('error_vendor');
		}
		
		if (!$this->request->getPostE('pp_pro_uk_user')) {
			$this->error['user'] = $this->language->get('error_user');
		}

		if (!$this->request->getPostE('pp_pro_uk_password')) {
			$this->error['password'] = $this->language->get('error_password');
		}

		if (!$this->request->getPostE('pp_pro_uk_partner')) {
