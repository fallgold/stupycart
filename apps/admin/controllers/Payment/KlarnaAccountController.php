...
class KlarnaAccountController extends \Stupycart\Admin\Controllers\ControllerBase {
    private $error = array();
    ...
    public function indexAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {            
			...
			foreach ($this->request->getPostE('klarna_account') as $klarna_account) {
        ...
        $this->data['breadcrumbs'][] = array(
            ...
            'href'      => $this->url->link('payment/klarna_account', 'token=' . $this->session->get('token'), 'SSL'),
            'separator' => ' :: '
        );
        ...
        $this->data['action'] = $this->url->link('payment/klarna_account', 'token=' . $this->session->get('token'), 'SSL');
		...
		if ($this->request->hasPost('klarna_account')) {
			$this->data['klarna_account'] = $this->request->getPostE('klarna_account');
		} else {
        ...
        $this->data['clear'] = $this->url->link('payment/klarna_account/clear', 'token=' . $this->session->get('token'), 'SSL'); 
    ...
    private function validate() {
        ...
        foreach ($this->request->getPostE('klarna_account') as $key => $klarna_account) {
            if ($klarna_account['status']) {
    ...
    public function clearAction() {
        ...
        $this->response->redirect($this->url->link('payment/klarna_account', 'token=' . $this->session->get('token'), 'SSL'), true);
		return;
    }
}
?>