...
class Parcelforce48Controller extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {   
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('parcelforce_48', $this->request->getPostE());		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('shipping/parcelforce_48', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('shipping/parcelforce_48', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/shipping', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('parcelforce_48_rate')) {
			$this->data['parcelforce_48_rate'] = $this->request->getPostE('parcelforce_48_rate');
		...
		if ($this->request->hasPost('parcelforce_48_insurance')) {
			$this->data['parcelforce_48_insurance'] = $this->request->getPostE('parcelforce_48_insurance');
		...
		if ($this->request->hasPost('parcelforce_48_display_weight')) {
			$this->data['parcelforce_48_display_weight'] = $this->request->getPostE('parcelforce_48_display_weight');
		} else {
			$this->data['parcelforce_48_display_weight'] = $this->config->get('parcelforce_48_display_weight');
		}

		if ($this->request->hasPost('parcelforce_48_display_insurance')) {
			$this->data['parcelforce_48_display_insurance'] = $this->request->getPostE('parcelforce_48_display_insurance');
		} else {
			$this->data['parcelforce_48_display_insurance'] = $this->config->get('parcelforce_48_display_insurance');
		}

		if ($this->request->hasPost('parcelforce_48_display_time')) {
			$this->data['parcelforce_48_display_time'] = $this->request->getPostE('parcelforce_48_display_time');
		} else {
			$this->data['parcelforce_48_display_time'] = $this->config->get('parcelforce_48_display_time');
		}
		
		if ($this->request->hasPost('parcelforce_48_tax_class_id')) {
			$this->data['parcelforce_48_tax_class_id'] = $this->request->getPostE('parcelforce_48_tax_class_id');
		} else {
		...
		if ($this->request->hasPost('parcelforce_48_geo_zone_id')) {
			$this->data['parcelforce_48_geo_zone_id'] = $this->request->getPostE('parcelforce_48_geo_zone_id');
		} else {
		...
		if ($this->request->hasPost('parcelforce_48_status')) {
			$this->data['parcelforce_48_status'] = $this->request->getPostE('parcelforce_48_status');
		} else {
			$this->data['parcelforce_48_status'] = $this->config->get('parcelforce_48_status');
		}
		
		if ($this->request->hasPost('parcelforce_48_sort_order')) {
			$this->data['parcelforce_48_sort_order'] = $this->request->getPostE('parcelforce_48_sort_order');
		} else {
			$this->data['parcelforce_48_sort_order'] = $this->config->get('parcelforce_48_sort_order');
		}				

		$this->view->pick('shipping/parcelforce_48');
		$this->_commonAction();
