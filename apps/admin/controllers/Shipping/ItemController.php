...
class ItemController extends \Stupycart\Admin\Controllers\ControllerBase { 
	private $error = array(); 
	...
	public function indexAction() {  
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('item', $this->request->getPostE());		
		...
		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('shipping/item', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('shipping/item', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/shipping', 'token=' . $this->session->get('token'), 'SSL');

		if ($this->request->hasPost('item_cost')) {
			$this->data['item_cost'] = $this->request->getPostE('item_cost');
		} else {
			$this->data['item_cost'] = $this->config->get('item_cost');
		}

		if ($this->request->hasPost('item_tax_class_id')) {
			$this->data['item_tax_class_id'] = $this->request->getPostE('item_tax_class_id');
		} else {
		...
		if ($this->request->hasPost('item_geo_zone_id')) {
			$this->data['item_geo_zone_id'] = $this->request->getPostE('item_geo_zone_id');
		} else {
		...
		if ($this->request->hasPost('item_status')) {
			$this->data['item_status'] = $this->request->getPostE('item_status');
		} else {
			$this->data['item_status'] = $this->config->get('item_status');
		}
		
		if ($this->request->hasPost('item_sort_order')) {
			$this->data['item_sort_order'] = $this->request->getPostE('item_sort_order');
		} else {
			$this->data['item_sort_order'] = $this->config->get('item_sort_order');
		}	

		$this->view->pick('shipping/item');
		$this->_commonAction();
