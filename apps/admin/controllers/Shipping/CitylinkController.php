...
class CitylinkController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {   
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('citylink', $this->request->getPostE());		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('shipping/citylink', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('shipping/citylink', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/shipping', 'token=' . $this->session->get('token'), 'SSL');
		
		if ($this->request->hasPost('citylink_rate')) {
			$this->data['citylink_rate'] = $this->request->getPostE('citylink_rate');
		...
		if ($this->request->hasPost('citylink_tax_class_id')) {
			$this->data['citylink_tax_class_id'] = $this->request->getPostE('citylink_tax_class_id');
		} else {
		...
		if ($this->request->hasPost('citylink_geo_zone_id')) {
			$this->data['citylink_geo_zone_id'] = $this->request->getPostE('citylink_geo_zone_id');
		} else {
		...
		if ($this->request->hasPost('citylink_status')) {
			$this->data['citylink_status'] = $this->request->getPostE('citylink_status');
		} else {
			$this->data['citylink_status'] = $this->config->get('citylink_status');
		}
		
		if ($this->request->hasPost('citylink_sort_order')) {
			$this->data['citylink_sort_order'] = $this->request->getPostE('citylink_sort_order');
		} else {
			$this->data['citylink_sort_order'] = $this->config->get('citylink_sort_order');
		}				

		$this->view->pick('shipping/citylink');
		$this->_commonAction();
