...
class FlatController extends \Stupycart\Admin\Controllers\ControllerBase {
	private $error = array(); 
	...
	public function indexAction() {   
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validate()) {
			$this->model_setting_setting->editSetting('flat', $this->request->getPostE());		
 		...
 		if (isset($this->error['warning'])) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('shipping/flat', 'token=' . $this->session->get('token'), 'SSL'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->link('shipping/flat', 'token=' . $this->session->get('token'), 'SSL');
		
		$this->data['cancel'] = $this->url->link('extension/shipping', 'token=' . $this->session->get('token'), 'SSL');
		
		if ($this->request->hasPost('flat_cost')) {
			$this->data['flat_cost'] = $this->request->getPostE('flat_cost');
		} else {
			$this->data['flat_cost'] = $this->config->get('flat_cost');
		}

		if ($this->request->hasPost('flat_tax_class_id')) {
			$this->data['flat_tax_class_id'] = $this->request->getPostE('flat_tax_class_id');
		} else {
		...
		if ($this->request->hasPost('flat_geo_zone_id')) {
			$this->data['flat_geo_zone_id'] = $this->request->getPostE('flat_geo_zone_id');
		} else {
		...
		if ($this->request->hasPost('flat_status')) {
			$this->data['flat_status'] = $this->request->getPostE('flat_status');
		} else {
			$this->data['flat_status'] = $this->config->get('flat_status');
		}
		
		if ($this->request->hasPost('flat_sort_order')) {
			$this->data['flat_sort_order'] = $this->request->getPostE('flat_sort_order');
		} else {
			$this->data['flat_sort_order'] = $this->config->get('flat_sort_order');
		}				

		$this->view->pick('shipping/flat');
		$this->_commonAction();
