...
class ManufacturerController extends \Stupycart\Admin\Controllers\ControllerBase { 
	private $error = array();
  	...
  	public function insertAction() {
		...
		if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
			$this->model_catalog_manufacturer->addManufacturer($this->request->getPostE());
			...
			$this->response->redirect($this->url->link('catalog/manufacturer', 'token=' . $this->session->get('token') . $url, 'SSL'), true);
		return;
		}
  	...
  	public function updateAction() {
    	...
    	if (($this->request->getServer('REQUEST_METHOD') == 'POST') && $this->validateForm()) {
			$this->model_catalog_manufacturer->editManufacturer($this->request->getQueryE('manufacturer_id'), $this->request->getPostE());
  	...
  	public function deleteAction() {
    	...
    	if ($this->request->hasPost('selected') && $this->validateDelete()) {
			foreach ($this->request->getPostE('selected') as $manufacturer_id) {
  	...
  	protected function getList() {
		...
		if ($this->request->hasQuery('page')) {
  		...
  		$this->data['breadcrumbs'] = array();
   		...
   		$this->data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('catalog/manufacturer', 'token=' . $this->session->get('token') . $url, 'SSL'),
      		'separator' => ' :: '
   		);
							
		$this->data['insert'] = $this->url->link('catalog/manufacturer/insert', 'token=' . $this->session->get('token') . $url, 'SSL');
		$this->data['delete'] = $this->url->link('catalog/manufacturer/delete', 'token=' . $this->session->get('token') . $url, 'SSL');	
    	...
    	foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/manufacturer/update', 'token=' . $this->session->get('token') . '&manufacturer_id=' . $result['manufacturer_id'] . $url, 'SSL')
			);
			...
			$this->data['manufacturers'][] = array(
				...
				'selected'        => $this->request->hasPost('selected') && in_array($result['manufacturer_id'], $this->request->getPostE('selected')),
				'action'          => $action
			);
		}	
		...
		$this->data['sort_name'] = $this->url->link('catalog/manufacturer', 'token=' . $this->session->get('token') . '&sort=name' . $url, 'SSL');
		$this->data['sort_sort_order'] = $this->url->link('catalog/manufacturer', 'token=' . $this->session->get('token') . '&sort=sort_order' . $url, 'SSL');
		...
		$pagination->url = $this->url->link('catalog/manufacturer', 'token=' . $this->session->get('token') . $url . '&page={page}', 'SSL');
		...
		$this->view->pick('catalog/manufacturer_list');
		$this->_commonAction();
  	...
  	protected function getForm() {
		...
		if (!$this->request->hasQuery('manufacturer_id')) {
			$this->data['action'] = $this->url->link('catalog/manufacturer/insert', 'token=' . $this->session->get('token') . $url, 'SSL');
		} else {
			$this->data['action'] = $this->url->link('catalog/manufacturer/update', 'token=' . $this->session->get('token') . '&manufacturer_id=' . $this->request->getQueryE('manufacturer_id') . $url, 'SSL');
		}
		
		$this->data['cancel'] = $this->url->link('catalog/manufacturer', 'token=' . $this->session->get('token') . $url, 'SSL');

    	if ($this->request->hasQuery('manufacturer_id') && ($this->request->getServer('REQUEST_METHOD') != 'POST')) {
		...
		if ($this->request->hasPost('manufacturer_store')) {
			$this->data['manufacturer_store'] = $this->request->getPostE('manufacturer_store');
		} elseif ($this->request->hasQuery('manufacturer_id')) {
			$this->data['manufacturer_store'] = $this->model_catalog_manufacturer->getManufacturerStores($this->request->getQueryE('manufacturer_id'));
		} else {
		...
		$this->view->pick('catalog/manufacturer_form');
		$this->_commonAction();
