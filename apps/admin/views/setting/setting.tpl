...
<div id="content">
  ...
  <div class="box">
    <div class="heading">
      <h1><img src="{$this->url->getBaseUrl()}image/setting.png" alt="" /> <?php echo $heading_title; ?></h1>
        ...
        <div id="tab-store">
          <table class="form">
            ...
            <tr>
              ...
              <td><select name="config_template" onchange="$('#template').load('{$this->url->getBaseUrl()}setting/setting/template&token=<?php echo $token; ?>?template=' + encodeURIComponent(this.value));">
...
$('#template').load('{$this->url->getBaseUrl()}setting/setting/template&token=<?php echo $token; ?>?template=' + encodeURIComponent($('select[name=\'config_template\']').attr('value')));
//--></script> 
<script type="text/javascript"><!--
$('select[name=\'config_country_id\']').bind('change', function() {
	$.ajax({
		url: '{$this->url->getBaseUrl()}setting/setting/country&token=<?php echo $token; ?>?country_id=' + this.value,
		dataType: 'json',
		beforeSend: function() {
			$('select[name=\'country_id\']').after('<span class="wait">&nbsp;<img src="{$this->url->getBaseUrl()}image/loading.gif" alt="" /></span>');
		},		
		complete: function() {
			$('.wait').remove();
		},			
