...
<script type="text/javascript" src="{$this->url->getBaseUrl()}javascript/jquery/jquery-1.7.1.min.js"></script>
<script type="text/javascript" src="{$this->url->getBaseUrl()}javascript/jquery/ui/jquery-ui-1.8.16.custom.min.js"></script>
<link rel="stylesheet" type="text/css" href="{$this->url->getBaseUrl()}javascript/jquery/ui/themes/ui-lightness/jquery-ui-1.8.16.custom.css" />
<script type="text/javascript" src="{$this->url->getBaseUrl()}javascript/jquery/ui/external/jquery.bgiframe-2.1.2.js"></script>
<script type="text/javascript" src="{$this->url->getBaseUrl()}javascript/jquery/jstree/jquery.tree.min.js"></script>
<script type="text/javascript" src="{$this->url->getBaseUrl()}javascript/jquery/ajaxupload.js"></script>
<style type="text/css">
body {
	padding: 0;
	margin: 0;
...
$(document).ready(function() { 
	...
	$('#column-right').bind('scrollstop', function() {
		$('#column-right a').each(function(index, element) {
			...
			if ((offset.top > 0) && (offset.top < height) && $(element).find('img').attr('src') == '<?php echo $no_image; ?>') {
				$.ajax({
					url: '{$this->url->getBaseUrl()}common/filemanager/image&token=<?php echo $token; ?>?image=' + encodeURIComponent('data/' + $(element).find('input[name=\'image\']').attr('value')),
					dataType: 'html',
	...
	$('#column-left').tree({
		data: { 
			...
			opts: { 
				method: 'post', 
				url: '{$this->url->getBaseUrl()}common/filemanager/directory?token=<?php echo $token; ?>'
			} 
		},
		selected: 'top',
		ui: {		
		...
		callback: {
			...
			onselect: function (NODE, TREE_OBJ) {
				$.ajax({
					url: '{$this->url->getBaseUrl()}common/filemanager/files?token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#create').bind('click', function() {
		...
		if (tree.selected) {
			...
			$('#dialog input[type=\'button\']').bind('click', function() {
				$.ajax({
					url: '{$this->url->getBaseUrl()}common/filemanager/create?token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#delete').bind('click', function() {
		...
		if (path) {
			$.ajax({
				url: '{$this->url->getBaseUrl()}common/filemanager/delete?token=<?php echo $token; ?>',
				type: 'post',
	...
	$('#move').bind('click', function() {
		...
		$('#dialog select[name=\'to\']').load('{$this->url->getBaseUrl()}common/filemanager/folders?token=<?php echo $token; ?>');
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {																
				$.ajax({
					url: '{$this->url->getBaseUrl()}common/filemanager/move?token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#copy').bind('click', function() {
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {																
				$.ajax({
					url: '{$this->url->getBaseUrl()}common/filemanager/copy?token=<?php echo $token; ?>',
					type: 'post',
	...
	$('#rename').bind('click', function() {
		...
		$('#dialog input[type=\'button\']').bind('click', function() {
			...
			if (path) {		
				$.ajax({
					url: '{$this->url->getBaseUrl()}common/filemanager/rename?token=<?php echo $token; ?>',
					type: 'post',
					data: 'path=' + encodeURIComponent(path) + '&name=' + encodeURIComponent($('#dialog input[name=\'name\']').val()),
					dataType: 'json',
					success: function(json) {
	...
	new AjaxUpload('#upload', {
		action: '{$this->url->getBaseUrl()}common/filemanager/upload?token=<?php echo $token; ?>',
		name: 'image',
		autoSubmit: false,
		responseType: 'json',
		...
		onSubmit: function(file, extension) {
			$('#upload').append('<img src="{$this->url->getBaseUrl()}image/loading.gif" class="loading" style="padding-left: 5px;" />');
		},
