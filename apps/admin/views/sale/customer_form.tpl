...
<div id="content">
  ...
  <div class="box">
    <div class="heading">
      <h1><img src="{$this->url->getBaseUrl()}image/customer.png" alt="" /> <?php echo $heading_title; ?></h1>
      ...
      <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form">
        <div id="tab-general">
          <div id="vtabs" class="vtabs"><a href="#tab-customer"><?php echo $tab_general; ?></a>
            ...
            <a href="#tab-address-<?php echo $address_row; ?>" id="address-<?php echo $address_row; ?>"><?php echo $tab_address . ' ' . $address_row; ?>&nbsp;<img src="{$this->url->getBaseUrl()}image/delete.png" alt="" onclick="$('#vtabs a:first').trigger('click'); $('#address-<?php echo $address_row; ?>').remove(); $('#tab-address-<?php echo $address_row; ?>').remove(); return false;" /></a>
            <?php $address_row++; ?>
            <?php } ?>
            <span id="address-add"><?php echo $button_add_address; ?>&nbsp;<img src="{$this->url->getBaseUrl()}image/add.png" alt="" onclick="addAddress();" /></span></div>
...
function country(element, index, zone_id) {
  if (element.value != '') {
		$.ajax({
			url: '{$this->url->getBaseUrl()}sale/customer/country&token=<?php echo $token; ?>?country_id=' + element.value,
			dataType: 'json',
			beforeSend: function() {
				$('select[name=\'address[' + index + '][country_id]\']').after('<span class="wait">&nbsp;<img src="{$this->url->getBaseUrl()}image/loading.gif" alt="" /></span>');
			},
			complete: function() {
				$('.wait').remove();
			},			
...
function addAddress() {	
	...
	$('#address-add').before('<a href="#tab-address-' + address_row + '" id="address-' + address_row + '"><?php echo $tab_address; ?> ' + address_row + '&nbsp;<img src="{$this->url->getBaseUrl()}image/delete.png" alt="" onclick="$(\'#vtabs a:first\').trigger(\'click\'); $(\'#address-' + address_row + '\').remove(); $(\'#tab-address-' + address_row + '\').remove(); return false;" /></a>');
...
$('#history').load('{$this->url->getBaseUrl()}sale/customer/history&token=<?php echo $token; ?>?customer_id=<?php echo $customer_id; ?>');

$('#button-history').bind('click', function() {
	$.ajax({
		url: '{$this->url->getBaseUrl()}sale/customer/history&token=<?php echo $token; ?>?customer_id=<?php echo $customer_id; ?>',
		type: 'post',
		dataType: 'html',
		...
		beforeSend: function() {
			...
			$('#history').before('<div class="attention"><img src="{$this->url->getBaseUrl()}image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
		complete: function() {
...
$('#transaction').load('{$this->url->getBaseUrl()}sale/customer/transaction&token=<?php echo $token; ?>?customer_id=<?php echo $customer_id; ?>');

$('#button-transaction').bind('click', function() {
	$.ajax({
		url: '{$this->url->getBaseUrl()}sale/customer/transaction&token=<?php echo $token; ?>?customer_id=<?php echo $customer_id; ?>',
		type: 'post',
		dataType: 'html',
		...
		beforeSend: function() {
			...
			$('#transaction').before('<div class="attention"><img src="{$this->url->getBaseUrl()}image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
		complete: function() {
...
$('#reward').load('{$this->url->getBaseUrl()}sale/customer/reward&token=<?php echo $token; ?>?customer_id=<?php echo $customer_id; ?>');

function addRewardPoints() {
	$.ajax({
		url: '{$this->url->getBaseUrl()}sale/customer/reward&token=<?php echo $token; ?>?customer_id=<?php echo $customer_id; ?>',
		type: 'post',
		dataType: 'html',
		...
		beforeSend: function() {
			...
			$('#reward').before('<div class="attention"><img src="{$this->url->getBaseUrl()}image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');
		},
		complete: function() {
...
function addBanIP(ip) {
	...
	$.ajax({
		url: '{$this->url->getBaseUrl()}sale/customer/addbanip?token=<?php echo $token; ?>',
		type: 'post',
		dataType: 'json',
		...
		beforeSend: function() {
			...
			$('.box').before('<div class="attention"><img src="{$this->url->getBaseUrl()}image/loading.gif" alt="" /> <?php echo $text_wait; ?></div>');		
		},
		complete: function() {
...
function removeBanIP(ip) {
	...
	$.ajax({
		url: '{$this->url->getBaseUrl()}sale/customer/removebanip?token=<?php echo $token; ?>',
		type: 'post',
		dataType: 'json',
		data: 'ip=' + encodeURIComponent(ip),
		beforeSend: function() {
