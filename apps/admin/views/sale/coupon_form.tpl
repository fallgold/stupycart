          ...
          <table class="form">
            ...
            <tr>
              ...
              <td><div id="coupon-product" class="scrollbox">
                  ...
                  <div id="coupon-product<?php echo $coupon_product['product_id']; ?>" class="<?php echo $class; ?>"> <?php echo $coupon_product['name']; ?><img src="{$this->url->getBaseUrl()}image/delete.png" alt="" />
            ...
            <tr>
              ...
              <td><div id="coupon-category" class="scrollbox">
                  ...
                  <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
                  <div id="coupon-category<?php echo $coupon_category['category_id']; ?>" class="<?php echo $class; ?>"> <?php echo $coupon_category['name']; ?><img src="{$this->url->getBaseUrl()}image/delete.png" alt="" />
...
$('input[name=\'category[]\']').bind('change', function() {
	...
	$.ajax({
		url: '{$this->url->getBaseUrl()}catalog/product/autocomplete&token=<?php echo $token; ?>&filter_category_id=' +  filter_category_id.value + '?limit=10000',
		dataType: 'json',
		success: function(json) {
			for (i = 0; i < json.length; i++) {
				if ($(filter_category_id).attr('checked') == 'checked') {
					...
					$('#coupon-product').append('<div id="coupon-product' + json[i]['product_id'] + '">' + json[i]['name'] + '<img src="{$this->url->getBaseUrl()}image/delete.png" alt="" /><input type="hidden" name="coupon_product[]" value="' + json[i]['product_id'] + '" /></div>');
				} else {
					$('#coupon-product' + json[i]['product_id']).remove();
				}			
			}
...
$('input[name=\'product\']').autocomplete({
	...
	select: function(event, ui) {
		...
		$('#coupon-product').append('<div id="coupon-product' + ui.item.value + '">' + ui.item.label + '<img src="{$this->url->getBaseUrl()}image/delete.png" alt="" /><input type="hidden" name="coupon_product[]" value="' + ui.item.value + '" /></div>');
...
$('input[name=\'category\']').autocomplete({
	...
	select: function(event, ui) {
		...
		$('#coupon-category').append('<div id="product-category' + ui.item.value + '">' + ui.item.label + '<img src="{$this->url->getBaseUrl()}image/delete.png" alt="" /><input type="hidden" name="coupon_category[]" value="' + ui.item.value + '" /></div>');
...
$('#history').load('{$this->url->getBaseUrl()}sale/coupon/history&token=<?php echo $token; ?>?coupon_id=<?php echo $coupon_id; ?>');
//--></script>
<?php } ?>
<script type="text/javascript"><!--
$('#tabs a').tabs(); 
