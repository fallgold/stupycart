...
<div id="content">
  ...
  <div class="box">
    <div class="heading">
      <h1><img src="{$this->url->getBaseUrl()}image/order.png" alt="" /> <?php echo $heading_title; ?></h1>
          ...
          <table class="list">
            ...
            <tbody id="product">
              ...
              <tr id="product-row<?php echo $product_row; ?>">
                <td class="center" style="width: 3px;"><img src="{$this->url->getBaseUrl()}image/delete.png" title="<?php echo $button_remove; ?>" alt="<?php echo $button_remove; ?>" style="cursor: pointer;" onclick="$('#product-row<?php echo $product_row; ?>').remove(); $('#button-update').trigger('click');" /></td>
          ...
          <table class="list">
            ...
            <tbody id="voucher">
              ...
              <tr id="voucher-row<?php echo $voucher_row; ?>">
                <td class="center" style="width: 3px;"><img src="{$this->url->getBaseUrl()}image/delete.png" title="<?php echo $button_remove; ?>" alt="<?php echo $button_remove; ?>" style="cursor: pointer;" onclick="$('#voucher-row<?php echo $voucher_row; ?>').remove(); $('#button-update').trigger('click');" /></td>
...
$('input[name=\'customer\']').catcomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: '{$this->url->getBaseUrl()}sale/customer/autocomplete&token=<?php echo $token; ?>?filter_name=' +  encodeURIComponent(request.term),
			dataType: 'json',
...
$('input[name=\'affiliate\']').autocomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: '{$this->url->getBaseUrl()}sale/affiliate/autocomplete&token=<?php echo $token; ?>?filter_name=' +  encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {	
				response($.map(json, function(item) {
					return {
...
$('select[name=\'payment_country_id\']').bind('change', function() {
	$.ajax({
		url: '{$this->url->getBaseUrl()}sale/order/country&token=<?php echo $token; ?>?country_id=' + this.value,
		dataType: 'json',
		beforeSend: function() {
			$('select[name=\'payment_country_id\']').after('<span class="wait">&nbsp;<img src="{$this->url->getBaseUrl()}image/loading.gif" alt="" /></span>');
		},
		complete: function() {
			$('.wait').remove();
		},			
...
$('select[name=\'payment_address\']').bind('change', function() {
	$.ajax({
		url: '{$this->url->getBaseUrl()}sale/customer/address&token=<?php echo $token; ?>?address_id=' + this.value,
		dataType: 'json',
		success: function(json) {
			if (json != '') {	
			...
			for (i = 0; i < ui.item.option.length; i++) {
				...
				if (option['type'] == 'file') {		
					new AjaxUpload('#button-option-' + option['product_option_id'], {
						action: '{$this->url->getBaseUrl()}sale/order/upload?token=<?php echo $token; ?>',
						name: 'file',
						autoSubmit: true,
						responseType: 'json',
						data: option,
						onSubmit: function(file, extension) {
							$('#button-option-' + (this._settings.data['product_option_id'] + '-' + this._settings.data['product_option_id'])).after('<img src="{$this->url->getBaseUrl()}image/loading.gif" class="loading" />');
						},
...
$('#button-product, #button-voucher, #button-update').live('click', function() {	
	...
	$.ajax({
		url: '<?php echo $store_url; ?>{$this->url->getBaseUrl()}checkout/manual?token=<?php echo $token; ?>',
		type: 'post',
		data: $(data),
		dataType: 'json',	
		beforeSend: function() {
		...
		success: function(json) {
			...
			if (json['order_product'] != '') {
				...
				for (i = 0; i < json['order_product'].length; i++) {
					...
					html += '  <td class="center" style="width: 3px;"><img src="{$this->url->getBaseUrl()}image/delete.png" title="<?php echo $button_remove; ?>" alt="<?php echo $button_remove; ?>" style="cursor: pointer;" onclick="$(\'#product-row' + product_row + '\').remove(); $(\'#button-update\').trigger(\'click\');" /></td>';
			...
			if (json['order_voucher'] != '') {
				var voucher_row = 0;
				 ...
				 for (i in json['order_voucher']) {
					...
					html += '  <td class="center" style="width: 3px;"><img src="{$this->url->getBaseUrl()}image/delete.png" title="<?php echo $button_remove; ?>" alt="<?php echo $button_remove; ?>" style="cursor: pointer;" onclick="$(\'#voucher-row' + voucher_row + '\').remove(); $(\'#button-update\').trigger(\'click\');" /></td>';
