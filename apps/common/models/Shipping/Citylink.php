...
class Citylink  extends \Libs\Stupy\Model {
	function getQuote($address) {
		$this->language->load('shipping/citylink');
		
		$query = $this->db_query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('citylink_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
