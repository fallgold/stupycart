...
class Voucher  extends \Libs\Stupy\Model {
	public function addVoucher($data) {
      	$this->db_query("INSERT INTO " . DB_PREFIX . "voucher SET code = '" . $this->db_escape($data['code']) . "', from_name = '" . $this->db_escape($data['from_name']) . "', from_email = '" . $this->db_escape($data['from_email']) . "', to_name = '" . $this->db_escape($data['to_name']) . "', to_email = '" . $this->db_escape($data['to_email']) . "', voucher_theme_id = '" . (int)$data['voucher_theme_id'] . "', message = '" . $this->db_escape($data['message']) . "', amount = '" . (float)$data['amount'] . "', status = '" . (int)$data['status'] . "', date_added = NOW()");
	}
	
	public function editVoucher($voucher_id, $data) {
      	$this->db_query("UPDATE " . DB_PREFIX . "voucher SET code = '" . $this->db_escape($data['code']) . "', from_name = '" . $this->db_escape($data['from_name']) . "', from_email = '" . $this->db_escape($data['from_email']) . "', to_name = '" . $this->db_escape($data['to_name']) . "', to_email = '" . $this->db_escape($data['to_email']) . "', voucher_theme_id = '" . (int)$data['voucher_theme_id'] . "', message = '" . $this->db_escape($data['message']) . "', amount = '" . (float)$data['amount'] . "', status = '" . (int)$data['status'] . "' WHERE voucher_id = '" . (int)$voucher_id . "'");
	}
	
	public function deleteVoucher($voucher_id) {
      	$this->db_query("DELETE FROM " . DB_PREFIX . "voucher WHERE voucher_id = '" . (int)$voucher_id . "'");
		$this->db_query("DELETE FROM " . DB_PREFIX . "voucher_history WHERE voucher_id = '" . (int)$voucher_id . "'");
	}
	
	public function getVoucher($voucher_id) {
      	$query = $this->db_query("SELECT DISTINCT * FROM " . DB_PREFIX . "voucher WHERE voucher_id = '" . (int)$voucher_id . "'");
		
		return $query->row;
	}

	public function getVoucherByCode($code) {
      	$query = $this->db_query("SELECT DISTINCT * FROM " . DB_PREFIX . "voucher WHERE code = '" . $this->db_escape($code) . "'");
	...
	public function getTotalVouchers() {
      	$query = $this->db_query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "voucher");
		
		return $query->row['total'];
	}	
	
	public function getTotalVouchersByVoucherThemeId($voucher_theme_id) {
      	$query = $this->db_query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "voucher WHERE voucher_theme_id = '" . (int)$voucher_theme_id . "'");
	...
	public function getVoucherHistories($voucher_id, $start = 0, $limit = 10) {
		...
		$query = $this->db_query("SELECT vh.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, vh.amount, vh.date_added FROM " . DB_PREFIX . "voucher_history vh LEFT JOIN `" . DB_PREFIX . "order` o ON (vh.order_id = o.order_id) WHERE vh.voucher_id = '" . (int)$voucher_id . "' ORDER BY vh.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}
	
	public function getTotalVoucherHistories($voucher_id) {
	  	$query = $this->db_query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "voucher_history WHERE voucher_id = '" . (int)$voucher_id . "'");
