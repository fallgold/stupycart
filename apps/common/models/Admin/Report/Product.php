...
class Product  extends \Libs\Stupy\Model {
	...
	public function getTotalProductsViewed() {
      	$query = $this->db_query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "product WHERE viewed > 0");
		
		return $query->row['total'];
	}
	
	public function getTotalProductViews() {
      	$query = $this->db_query("SELECT SUM(viewed) AS total FROM " . DB_PREFIX . "product");
		
		return $query->row['total'];
	}
			
	public function reset() {
		$this->db_query("UPDATE " . DB_PREFIX . "product SET viewed = '0'");
	}
