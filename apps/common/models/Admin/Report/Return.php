...
class Return  extends \Libs\Stupy\Model {
	public function getReturns($data = array()) {
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= " AND DATE(r.date_added) >= '" . $this->db_escape($data['filter_date_start']) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(r.date_added) <= '" . $this->db_escape($data['filter_date_end']) . "'";
		}
	...
	public function getTotalReturns($data = array()) {
		...
		if (!empty($data['filter_date_start'])) {
			$sql .= " AND DATE(date_added) >= '" . $this->db_escape($data['filter_date_start']) . "'";
		}

		if (!empty($data['filter_date_end'])) {
			$sql .= " AND DATE(date_added) <= '" . $this->db_escape($data['filter_date_end']) . "'";
		}
