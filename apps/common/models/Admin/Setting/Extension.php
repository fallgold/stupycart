...
class Extension  extends \Libs\Stupy\Model {
	...
	public function install($type, $code) {
		$this->db_query("INSERT INTO " . DB_PREFIX . "extension SET `type` = '" . $this->db_escape($type) . "', `code` = '" . $this->db_escape($code) . "'");
	}
	
	public function uninstall($type, $code) {
		$this->db_query("DELETE FROM " . DB_PREFIX . "extension WHERE `type` = '" . $this->db_escape($type) . "' AND `code` = '" . $this->db_escape($code) . "'");
	}
