...
final class Tax extends \Phalcon\DI\Injectable {
	...
	public function __construct($registry) {
		...
		if ($this->session->has('shipping_country_id') || $this->session->has('shipping_zone_id')) {
			$this->setShippingAddress($this->session->get('shipping_country_id'), (($_tmp = $this->session->get('shipping_zone_id')) ? $_tmp : null));
		} elseif ($this->config->get('config_tax_default') == 'shipping') {
			$this->setShippingAddress($this->config->get('config_country_id'), $this->config->get('config_zone_id'));
		}
		
		if ($this->session->has('payment_country_id') || $this->session->has('payment_zone_id')) {
			$this->setPaymentAddress($this->session->get('payment_country_id'), (($_tmp = $this->session->get('payment_zone_id')) ? $_tmp : null));
	...
	public function getRateName($tax_rate_id) {
		$tax_query = $this->ocdb->db_query("SELECT name FROM " . DB_PREFIX . "tax_rate WHERE tax_rate_id = '" . (int)$tax_rate_id . "'");
